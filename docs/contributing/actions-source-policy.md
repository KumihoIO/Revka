...
|----------|------|---------|
| CI | `.github/workflows/ci-run.yml` | Pull requests to `main` |
...
- 2026-06-22: Consolidated the two PR workflows — folded `Quality Gate` (`checks-on-pr.yml`)'s `security`/`check-32bit`/`Security Required Gate` jobs into `CI` (`ci-run.yml`) and deleted `checks-on-pr.yml`, so `lint`/`test`/`build` run once per PR instead of twice (#432). No allowlist change — the same pinned actions remain in use.
