...
This document is the canonical reference for channel configuration in Revka.
...
6. **Stale runtime state**: config changed but `revka daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.revka/config.toml`.
...
When running `revka channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `revka models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

Revka supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `revka channel list`, `revka channel doctor`, and `revka channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@revka:matrix.example.com"   # optional, recommended for E2EE
...
Revka supports two WhatsApp backends:
...
allowed_users = ["*"]
# bot_name = "revka"  # display name of the bot; filters own messages to prevent feedback loops
```
...
- `REVKA_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `REVKA_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `revka daemon` after config changes.
