...
- **受众：** 在代理网络中运行 Revka 的运维人员和维护者。
- **范围：** `proxy_config` 操作、模式选择、验证流程和故障排除。
- **非目标：** Revka 运行时行为之外的通用网络调试。
...
| 范围 | 影响 | 导出环境变量 | 典型用途 |
|---|---|---|---|
| `revka` | Revka 内部 HTTP 客户端 | 否 | 无进程级副作用的正常运行时代理 |
...
## 4. 模式 A — 仅代理 Revka 内部流量

当 Revka 提供商/渠道/工具 HTTP 流量应使用代理，但不导出进程级代理环境变量时使用。
