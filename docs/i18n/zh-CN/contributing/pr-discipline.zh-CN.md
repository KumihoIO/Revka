...
- 如果不可避免需要类似身份的上下文，仅使用 Revka 范围的角色/标签（例如 `RevkaAgent`、`RevkaOperator`、`revka_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`RevkaAgent`、`RevkaOperator`、`RevkaMaintainer`、`revka_user`
    - 服务/运行时标签：`revka_bot`、`revka_service`、`revka_runtime`、`revka_node`
    - 环境标签：`revka_project`、`revka_workspace`、`revka_channel`
