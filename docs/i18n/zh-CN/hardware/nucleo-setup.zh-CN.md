# Nucleo-F401RE 上的 Revka — 分步指南

在 Mac 或 Linux 主机上运行 Revka。通过 USB 连接 Nucleo-F401RE。通过 Telegram 或 CLI 控制 GPIO（LED、引脚）。

---

## 通过 Telegram 获取开发板信息（无需固件）

Revka 可以通过 USB 从 Nucleo 读取芯片信息，**无需烧录任何固件**。向你的 Telegram 机器人发送消息：
...
| 烧录命令 | `revka peripheral flash-nucleo` | 构建固件，通过 probe-rs 烧录 |
...
| 5 | `revka daemon` 或 `revka agent -m "Turn on LED"` |
...
- **GPIO 命令被忽略** — 检查配置中的 `path` 与你的串口匹配。运行 `revka peripheral list` 验证。
