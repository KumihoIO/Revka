# Revka 선언형 워크플로 — HOW-TO 가이드

> 원문: [`WORKFLOWS.md`](../../../WORKFLOWS.md). 이 문서는 한국어 번역본이며, 정식 명세는 영문이 기준입니다. 동작 변경이 영문에 먼저 반영될 수 있으니 의심스러우면 원문을 함께 확인하세요.

## 개요

Revka 워크플로는 다중 단계·다중 에이전트 파이프라인을 정의하는 YAML 파일입니다. Operator는 이를 결정적으로 실행합니다 — 데이터를 resolve하고, 에이전트를 띄우고, 조건에 따라 분기하고, 엔티티를 발행하고, 다운스트림 워크플로로 체이닝하는 전 과정을 수동 오케스트레이션 없이 처리합니다.
...
| 우선순위 | 경로 | 용도 |
|----------|------|---------|
| 3 (가장 높음) | `.revka/workflows/` | 프로젝트 로컬 오버라이드 |
| 2 | `~/.revka/workflows/` | 사용자 전역 워크플로 |
| 1 (가장 낮음) | `operator/workflow/builtins/` | 기본 내장 워크플로 |

뒤쪽 소스가 앞쪽을 덮어씁니다. 디스크에서 워크플로를 찾지 못하면 Operator는 마지막 폴백으로 **Kumiho** (`Revka/Workflows` 스페이스)를 확인합니다.
...
- **Cron**: `triggers:` 블록 추가 — 저장 시 Revka가 스케줄을 자동 등록
...
`agent.output_fields`가 설정되어 있으면 Revka는 이를 필수 구조화 출력
계약으로 처리합니다:
...
- id: resolve_cursor
  ...
  resolve:
    ...
    space: ""                  # 스페이스 경로 필터 (기본: Revka/WorkflowOutputs)
...
- id: report
  ...
  output:
    ...
    entity_tag: "ready"
    entity_space: "Revka/WorkflowOutputs"   # 기본 스페이스
    entity_metadata:
      topic: "${inputs.topic}"
...
UI를 통해 cron 트리거가 있는 워크플로를 Kumiho에 저장하면 Revka가 스케줄 잡으로 자동 등록합니다. 스케줄 시각이 되면 cron 스케줄러가 직접 `POST /api/workflows/run/{name}`을 호출합니다.
...
이벤트 리스너는 `/Revka/WorkflowOutputs`에서 발생하는 `revision.tagged` 이벤트를 감시합니다. output 단계가 트리거 규칙과 일치하는 엔티티를 발행하면 다운스트림 워크플로가 자동 실행됩니다.
...
3. **YAML이 디스크에 기록됨** — `~/.revka/workflows/{slug}.r{N}.yaml`
...
활성화되면 executor는 각 단계 완료 시점과 워크플로 일시 정지(human approval) 시점에 상태를 `~/.revka/workflow_checkpoints/{run_id}.json`에 저장합니다. 체크포인트는 승인 후 재개와 실패 실행 재시도 같은 명시적 사용자 동작을 지원합니다. Operator 시작 시 중단된 워크플로 실행을 자동으로 재개하지 않습니다. 진행 중으로 남은 오래된 실행은 실패로 표시되며, 사용자가 Retry를 눌러야 재시도 경로가 시작됩니다.
...
확인할 것: YAML이 `~/.revka/workflows/`에 있거나, 아티팩트와 함께 Kumiho에 등록되어 있나요? Operator는 디스크를 먼저 확인한 뒤 `kref://Revka/Workflows/my-workflow.workflow`로 resolve합니다.
...
3. 엔티티가 기대한 스페이스(기본: `Revka/WorkflowOutputs`)에 발행되었는가
...
이는 아티팩트가 첨부되기 전에 리비전이 `published`로 태깅되면 발생합니다. Revka v2026.4.21+에서는 아티팩트를 첨부한 뒤에 publish하도록 수정되어 있습니다.
