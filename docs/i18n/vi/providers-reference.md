...
3. Biến môi trường dự phòng chung: `REVKA_API_KEY`, sau đó là `API_KEY`
...
- Sau khi chuẩn hóa multimodal, Revka gửi payload hình ảnh qua trường `messages[].images` gốc của Ollama.
- Nếu chọn provider không hỗ trợ vision, Revka trả về lỗi rõ ràng thay vì âm thầm bỏ qua hình ảnh.
