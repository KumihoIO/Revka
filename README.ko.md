...
| 스텝 타입 | 설명 |
|-----------|-------------|
| `agent` | prompt, role, model, tools, timeout을 지정하여 Construct 에이전트(claude/codex/google_agents)를 스폰 |
...
|-------|-------------|
| `name` | 고유 식별자 |
| `agent_type` | `claude`, `codex` 또는 `google_agents` |
...
| `soul` | 성격 및 가치관 |
| `tone` | 커뮤니케이션 스타일 |
| `model` | 지원되는 CLI의 모델 오버라이드(Google Agents CLI는 보통 ADK 프로젝트 설정을 사용) |
