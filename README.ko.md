...
Construct는 엔터프라이즈 AgentOps도 1급 목표로 설계되었습니다. 기존
Claude/Codex 워크플로우 에이전트가 거버넌스 적용 툴 표면을 통해
Google Agents CLI를 사용하고, A2A로 외부 시스템과 조율하며, Gemini/ADK
기반 에이전트를 Google Cloud Run에 패키징할 수 있습니다. IAM, 서비스
계정, bearer-token 호출 인증, 로깅, 롤백, 증거 gate까지 함께 다룹니다.
...
**Operator**는 Construct의 조종간입니다 — 선언적 YAML 워크플로우를 20개 스텝 타입과 여러 고급 오케스트레이션 패턴을 통해 구동하는 Python MCP 서버입니다. 에이전트는 Construct 안에서 실행되고, Operator는 전체 보드를 봅니다.
...
| `python` | JSON I/O로 Python 스크립트 또는 인라인 Python 실행 |
| `email` | 선택적 클릭 추적과 dry-run을 지원하는 SMTP outbound email 전송 |
...
| `notify` | 하나 이상의 채널로 fire-and-forget 알림 전송 |
| `output` | 템플릿 보간을 지원하는 구조화된 출력(text/json/markdown) 생성 |

`action: notify` 같은 에디터 액션은 로딩 시 `workflow/schema.py`의
`ACTION_DEFAULTS`로 해석됩니다. `notify` 자체도 실제 `StepType`입니다.
...
### Google AgentOps 및 엔터프라이즈 A2A

Construct는 Google Agents CLI를 별도의 Construct 에이전트 프로바이더로
취급하지 않고, 기존 워크플로우 에이전트가 사용할 수 있는 거버넌스
적용 **툴 기능**으로 통합합니다. `claude` 또는 `codex` 워크플로우 단계는
축소된 Google AgentOps 툴 표면을 요청하고, ADK / Agent Platform
라이프사이클 작업에 `agents-cli`를 호출할 수 있습니다. 이때도 워크플로우
상태, 승인, 로그, 아티팩트, 롤백 증거는 Construct가 계속 소유합니다.
...
- id: agentops-plan
  ...
  agent:
    ...
    prompt: |
      Google Agents CLI와 A2A 툴링을 사용해 승인과 롤백이 포함된
      엔터프라이즈 사고 대응 에이전트 배포 계획을 작성하라.
```

워크플로우 UI에서 `google_agents_cli`를 추가하면 Construct는 companion
A2A 툴(`a2a_discover`, `a2a_send_task`, `a2a_get_remote_task`)을 자동으로
확장하고, 단계 실행 전 축소된 MCP 표면을 preflight로 검증합니다. 더 넓은
Operator MCP 툴 세트가 필요할 때만 `agent.tools: all`을 사용하세요.

[`examples/google-agents-track3/construct-agentops-a2a`](examples/google-agents-track3/construct-agentops-a2a)
아래의 Google Track 3 패키지는 같은 기능을 엔터프라이즈 B2B 배포 형태로
보여줍니다:

- **클라우드 네이티브 런타임** — Google Cloud Run에 배포 가능하며,
  서비스 계정, IAM, 요청 제한, 태스크 보존, 타임아웃, Cloud Logging,
  secret 기반 A2A bearer-token 인증을 위한 프로덕션 컨트롤을 포함합니다.
- **Google 기반 지능** — Vertex AI의 Gemini와 Google ADK 오케스트레이션을
  기준으로 패키징되어 있습니다.
- **A2A 상호운용성** — `/.well-known/agent-card.json`,
  `/agent-card.json`, JSON-RPC `agent/card`, `message/send`, `tasks/get`,
  `tasks/list`, `tasks/cancel`을 노출합니다.
- **엔터프라이즈 거버넌스 스토리** — 비즈니스 영향, 전문 에이전트
  핸드오프, Google Cloud 증거, 승인 경계, 롤백 경로, 운영자 권고를 포함한
  사고 계획을 반환합니다.
- **녹화/증거 게이트** — 결정론적 소스 probe와 Track 3 증거 gate가
  Cloud Run 배포, A2A 호출, Gemini/ADK 런타임 메타데이터, B2B 패키징,
  거버넌스, Gemini Enterprise 등록 준비성을 검증합니다.

엔터프라이즈 준비 상태 요약:

| 준비 영역 | Construct 증거 |
|-----------|----------------|
| 클라우드 배포 | Cloud Run 패키징, 배포 스크립트, 소스 probe, 녹화 runbook |
| ID 및 접근 제어 | 전용 서비스 계정, IAM 경계, bearer-token A2A 호출, secret 기반 설정 |
| 상호운용성 | 공개 A2A agent card와 task send/get/list/cancel 라이프사이클 |
| 거버넌스 | 워크플로우 승인, 롤백 증거, 운영자 권고, 감사 아티팩트 |
| 관측성 | Construct 실행 trace, 캡처된 증거 아티팩트, Cloud Logging 통합 |
| B2B 패키징 | 소프트웨어 및 IT 운영팀을 위한 "Construct Enterprise AgentOps Control Plane" 내러티브 |

Cloud Run 녹화 runbook은
[`docs/ops/google-agents-track3-enterprise-readiness.md`](docs/ops/google-agents-track3-enterprise-readiness.md)를,
`google_agents_cli` 안전성 및 데모 준비 매트릭스는
[`docs/ops/google-agents-cli-demo-readiness.md`](docs/ops/google-agents-cli-demo-readiness.md)를
참고하세요.
...
| 오케스트레이션 | Python 3.11+ Operator MCP 서버 (20개 스텝 타입, map-reduce / supervisor / group-chat / handoff / refinement 패턴) |
