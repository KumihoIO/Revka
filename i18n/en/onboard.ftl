...
memory-kumiho-token-existing = Paste an existing Kumiho service token
memory-kumiho-token-skip = Skip token setup for now
memory-kumiho-cloud-failed = Kumiho Cloud onboarding failed: {$err}
memory-kumiho-paste-existing = Paste an existing Kumiho service token instead?
memory-kumiho-token-hidden-note = Paste the token, then press Enter. Input is hidden for security, so pasted text will not appear on screen.
memory-kumiho-token-captured = Token captured ({$chars} characters, hidden)
memory-kumiho-token-concatenated-warning = This looks like multiple JWT tokens pasted together. Paste exactly one Kumiho service token.
memory-kumiho-token-jwt-warning = This looks like a Firebase/Auth JWT, not a Kumiho service token. Using it as KUMIHO_SERVICE_TOKEN will cause Kumiho 401 errors.
memory-kumiho-token-use-anyway = Use this token anyway?
memory-kumiho-token-retry = Paste a Kumiho service token generated from Kumiho Cloud Service Tokens.
...
memory-kumiho-payment-browser = Complete payment in your browser, then return here:
memory-kumiho-checkout-enter = Press Enter after checkout is complete
memory-kumiho-checkout-unverified = Checkout is not verified yet: {$err}
memory-kumiho-checkout-retry = Try signup verification again?
memory-kumiho-token-name = Service token name
memory-kumiho-tier = Kumiho tier
memory-kumiho-region = Kumiho region
memory-kumiho-checkout-required = checkout required
memory-kumiho-tenant = Kumiho tenant: {$tenant}
