...
welcome-title = Revka에 오신 것을 환영합니다.
...
provider-api-base-prompt = API 엔드포인트 URL (예: http://localhost:1234 — Revka가 LLM 요청을 보낼 주소)
...
hardware-prompt = Revka가 어떻게 물리 세계와 상호작용할까요? (시리얼 장치, 없음 등)
...
memory-kumiho-managed-default = Revka는 기본적으로 관리형 Kumiho Cloud 엔드포인트를 사용합니다.
memory-kumiho-no-endpoint = 직접 호스팅하지 않는 한 Kumiho HTTP 엔드포인트를 입력할 필요가 없습니다.
memory-kumiho-token-setup = Kumiho 토큰 설정
memory-kumiho-token-cloud = Kumiho Cloud에 가입/로그인하고 토큰 발급
memory-kumiho-token-existing = 기존 Kumiho 서비스 토큰 붙여넣기
memory-kumiho-token-skip = 지금은 토큰 설정 건너뛰기
memory-kumiho-cloud-failed = Kumiho Cloud 온보딩 실패: {$err}
...
memory-kumiho-api-managed = 관리형 기본값
memory-kumiho-token-configured = 설정됨
memory-kumiho-token-not-configured = 설정되지 않음
memory-kumiho-backend-select = Kumiho 백엔드
memory-kumiho-backend-cloud = 관리형 Kumiho Cloud (권장) — 관리형, 여러 기기에서 사용
memory-kumiho-backend-localce = 로컬 Community Edition — 이 컴퓨터에서 직접 호스팅 (무료, 단일 사용자)
memory-kumiho-ce-ready = 로컬, 준비 완료 ({$url})
memory-kumiho-ce-configured = {$url}에 구성됨 — CE 설정을 마친 뒤 `revka doctor`를 실행하세요
memory-kumiho-ce-failed = 로컬 CE 설정을 완료하지 못했습니다: {$err}
memory-kumiho-account-email = Kumiho 계정 이메일
memory-kumiho-account-password = Kumiho 계정 비밀번호
memory-kumiho-existing-tenant = 기존 Kumiho 테넌트: {$tenant}
memory-kumiho-token-for-existing = 이 테넌트용 Revka 서비스 토큰을 발급할까요?
memory-kumiho-display-name = 표시 이름 (선택)
memory-kumiho-display-name-warning = 표시 이름을 업데이트하지 못했습니다: {$err}
memory-kumiho-org-name = 조직/워크스페이스 이름 (선택)
memory-kumiho-payment-browser = 브라우저에서 결제를 완료한 뒤 여기로 돌아오세요:
memory-kumiho-checkout-enter = 결제가 완료되면 Enter를 누르세요
memory-kumiho-checkout-unverified = 아직 결제가 확인되지 않았습니다: {$err}
memory-kumiho-checkout-retry = 가입 검증을 다시 시도할까요?
memory-kumiho-token-name = 서비스 토큰 이름
memory-kumiho-tier = Kumiho 티어
memory-kumiho-region = Kumiho 리전
memory-kumiho-checkout-required = 결제 필요
memory-kumiho-tenant = Kumiho 테넌트: {$tenant}
...
reinit-banner = ⚠️  Revka 설정을 초기화합니다...
...
err-try-onboard = 워크스페이스를 초기화하려면 `revka onboard`를 실행하세요.
...
gemini-cli-reuse-info = Revka가 기존 Gemini CLI 인증을 재사용합니다.
...
tool-mode-info-1 = Revka가 외부 앱과 어떻게 연결될지 선택하세요.
...
composio-info = Revka는 Composio를 도구로 사용합니다 — 핵심 에이전트는 로컬에 유지됩니다.
...
secrets-info-1 = Revka는 config.toml에 저장된 API 키를 암호화할 수 있습니다.
...
hardware-info-1 = Revka는 물리 하드웨어(LED, 센서, 모터 등)와 통신할 수 있습니다.
...
channels-info-1 = 채널을 통해 어디서든 Revka와 대화할 수 있습니다.
...
notify-target-prompt = 발신 알림용 알림 대상 (선택, 건너뛰려면 Enter)

## ── Telegram ──────────────────────────────────────────────────────

telegram-title = Telegram 설정
telegram-subtitle = Telegram에서 Revka와 대화하기
...
telegram-notify-info = 워크플로 알림과 승인 요청을 보낼 알림 대상을 설정하세요.
telegram-notify-detect-prompt = 지금 봇에게 메시지를 보낸 뒤, 계속해서 채팅 ID를 자동 감지할까요?
telegram-notify-detected = 감지된 채팅 ID: {$chat_id}
telegram-notify-detect-empty = 최근 메시지를 찾지 못했습니다 — 아래에 채팅 ID를 직접 입력하세요.
telegram-notify-detect-conflict = 데몬이 이미 폴링 중입니다 — 아래에 채팅 ID를 직접 입력하세요.
telegram-notify-manual-prompt = 알림 채팅 ID (선택, 건너뛰려면 Enter)

## ── Discord ───────────────────────────────────────────────────────

discord-title = Discord 설정
discord-subtitle = Discord에서 Revka와 대화하기
...
discord-notify-list-prompt = 이 서버의 텍스트 채널 목록에서 알림 대상을 선택할까요?
discord-notify-select-prompt = 알림 채널
discord-notify-list-failed = 채널 목록을 가져오지 못했습니다 — 아래에 채널 ID를 직접 입력하세요.
discord-notify-manual-prompt = 알림 채널 ID (선택, 건너뛰려면 Enter)

## ── Slack ─────────────────────────────────────────────────────────

slack-title = Slack 설정
slack-subtitle = Slack에서 Revka와 대화하기
...
slack-notify-default = 기본 채널 {$channel}을(를) 알림 대상으로 사용합니다.
slack-notify-prompt = 알림 채널 ID (선택, 건너뛰려면 Enter)
...
imessage-info-1 = Revka는 iMessage 데이터베이스를 읽고 AppleScript로 답장합니다.
...
lark-subtitle = {$provider}에서 Revka와 대화하기
...
nostr-info-1 = Revka가 Nostr 릴레이에서 암호화된 DM을 수신합니다.
