"""A2A server for the coder agent — speaks Revka's A2A dialect.
...
AGENT_NAME = "Revka Coder Agent"
AGENT_DESCRIPTION = (
    "ADK/Gemini coding executor: clones a GitHub repository, implements an "
    "issue fix per the given strategy, runs tests, and opens a pull request. "
    "Returns JSON {pr_url, branch, summary, test_status}."
)
SKILL = {
    "id": "coder-implement-fix",
    "name": "Implement GitHub issue fix",
    "description": (
        "Input JSON: {repo_name, issue_number, issue_title, issue_body, strategy}. "
        "Clones the repo, implements the fix, runs pytest when detectable, pushes "
        "a branch and opens a PR. Output JSON: {pr_url, branch, summary, test_status}."
    ),
    "tags": ["coding", "github", "pull-request", "adk", "gemini", "a2a"],
    "examples": [
        '{"repo_name": "acme/api", "issue_number": 42, "issue_title": "Fix crash", '
        '"issue_body": "...", "strategy": "guard against None in handler"}'
    ],
...
TASK_TIMEOUT_SECONDS = float(os.getenv("TASK_TIMEOUT_SECONDS", "1500"))

logger = logging.getLogger("coder-a2a")
...
async def _execute_task(task_id: str, payload: dict[str, Any], raw_text: str) -> None:
    async with _EXEC_LOCK:
        ...
        workdir = tempfile.mkdtemp(prefix="coder-task-")
        try:
            ...
            _finish_task(task_id, "completed", result, artifact_name="coder-result")
            logger.info("task %s completed", task_id)
        except asyncio.CancelledError:
            _finish_task(task_id, "canceled", "task canceled", artifact_name="coder-result")
            raise
