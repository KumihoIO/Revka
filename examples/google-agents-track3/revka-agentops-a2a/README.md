...
This example packages Revka as a B2B AgentOps control-plane agent. It
...
export PROJECT_ID=your-google-cloud-project
export REGION=us-central1
export SERVICE_ACCOUNT="revka-agentops-a2a@$PROJECT_ID.iam.gserviceaccount.com"

gcloud iam service-accounts create revka-agentops-a2a \
  --project "$PROJECT_ID" \
  --display-name "Revka AgentOps A2A"

printf '%s' "$A2A_BEARER_TOKEN" | gcloud secrets create revka-a2a-bearer-token \
  --project "$PROJECT_ID" \
  --data-file=-

gcloud secrets add-iam-policy-binding revka-a2a-bearer-token \
  --project "$PROJECT_ID" \
