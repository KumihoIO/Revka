...
use revka::config::{AutonomyConfig, ChannelsConfig, Config, GatewayConfig, SecurityConfig};
...
fn gateway_path_prefix_rejects_missing_leading_slash() {
    ...
    config.gateway.path_prefix = Some("revka".into());
...
fn gateway_path_prefix_accepts_valid_prefixes() {
    for prefix in ["/revka", "/apps/revka", "/api/hassio_ingress/abc123"] {
        let mut config = Config::default();
...
"#;
    ...
    assert_eq!(
        ...
        Some("~/.revka/state/whatsapp-web/session.db")
    );
