 ...
 * editor's "Import YAML" path. Exercises the cases that broke under the old
 * regex-based parser:
 *   - canonical `conditional.branches` (PR #216, #217)
 *   - legacy flat `condition` + `on_true` + `on_false`
 *   - multi-line `prompt: |` scalars
 *   - hyphenated step IDs
 *   - mixed indentation
 *   - deeply nested structures
 *   - malformed YAML throws (caught by handleYamlImport)
 *
 * Run: npx tsx --test src/components/workflows/__tests__/yamlSync.test.ts
 */
...
test('full multi-step workflow: nodes populated and edges reconstruct', () => {
  // Mirror of the construct-blog-writer shape — agent → conditional →
  // parallel → for_each → output, with hyphenated IDs and multi-line prompts.
  const yaml = `
name: blog-writer
version: "1.0"
...
`;
  ...
  // Conditional flat fields are populated from canonical branches.
  ...
  // Edges reconstruct: depends_on, gate branches, parallel children, output ref.
  const { edges } = tasksToFlow(tasks);
  const pairs = new Set(edges.map((e) => `${e.source}->${e.target}`));
