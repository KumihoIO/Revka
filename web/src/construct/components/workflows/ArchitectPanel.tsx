...
function buildContextPreface(workflowName: string): string {
  return [
    ...
    '    depends_on: [research_a, research_b]',
    '  The runtime cannot auto-inject prior results into prompts. If a step needs information from another step, the prompt MUST contain `${other_step.output}` (or `${other_step.output_data.field}`). The validator now rejects agent steps with depends_on entries that are not referenced in the prompt.',
    '',
