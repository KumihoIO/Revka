...
function buildContextPreface(workflowName: string): string {
  return [
    ...
    '- If no fit AND the user described a recurring named role, call `save_agent_template` with required fields: `name` (kebab-case), `agent_type` (claude|codex|google_agents), `role` (one of: coder, reviewer, researcher, tester, architect, planner), `capabilities` (string array of tags), `description`. Optional: `system_hint`, `identity`, `soul`, `tone`, `model`. Then reference the new template via `agent.template`.',
    '- Use `agent_type: google_agents` for Google Agents CLI / ADK projects. The ADK project controls its model; keep `agent.tools: none` unless a Claude/Codex wrapper step needs Construct MCP tools.',
    '- Skip persona creation for one-shot generic steps; bare `agent_type: claude`, `agent_type: codex`, or `agent_type: google_agents` (no template) is correct there.',
