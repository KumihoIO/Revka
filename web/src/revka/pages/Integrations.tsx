...
export default function Integrations() {
  ...
  const statusBadge = (status: Integration['status']) => {
    switch (status) {
      case 'Active':
        return { icon: Check, label: t('integrations.badge.active'), color: 'var(--revka-signal-selected)', border: 'color-mix(in srgb, var(--revka-signal-selected) 24%, transparent)', bg: 'color-mix(in srgb, var(--revka-signal-selected) 10%, transparent)' };
      case 'Available':
        return { icon: Zap, label: t('integrations.badge.available'), color: 'var(--revka-signal-network)', border: 'color-mix(in srgb, var(--revka-signal-network) 24%, transparent)', bg: 'color-mix(in srgb, var(--revka-signal-network) 10%, transparent)' };
      case 'ComingSoon':
        return { icon: Clock, label: t('integrations.badge.coming_soon'), color: 'var(--revka-text-faint)', border: 'var(--revka-border-soft)', bg: 'transparent' };
      default:
        return { icon: Clock, label: status, color: 'var(--revka-text-faint)', border: 'var(--revka-border-soft)', bg: 'transparent' };
    }
  };
        ...
        <div className="grid gap-4 grid-cols-1 lg:min-h-0 lg:flex-1 lg:grid-cols-[18rem_minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
          <div className="flex flex-col gap-4 lg:overflow-y-auto lg:min-h-0">
            <Panel className="p-4" variant="secondary">
              <div className="revka-kicker">{t('integrations.coverage')}</div>
              <div className="mt-4 grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                <MetricPill label={t('integrations.metric.active')} value={summary.active} tone="var(--revka-signal-selected)" />
                <MetricPill label={t('integrations.metric.available')} value={summary.available} tone="var(--revka-signal-network)" />
                <MetricPill label={t('integrations.metric.coming_soon')} value={summary.comingSoon} tone="var(--revka-text-faint)" />
              </div>
            </Panel>

            <Panel className="p-4" variant="secondary">
              <div className="revka-kicker">{t('integrations.categories')}</div>
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderColor: activeCategory === category ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)',
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{count}</span>
                      </div>
                    </button>
                  );
                })}
          ...
          <Panel className="p-5 lg:overflow-y-auto">
            <div className="revka-kicker">{t('integrations.workspace')}</div>
                ...
                filtered.map((integration) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderColor: selectedIntegration?.name === integration.name ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)',
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-2">
                          <Puzzle className="h-4 w-4 shrink-0" style={{ color: 'var(--revka-signal-network)' }} />
                          <h4 className="truncate text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{integration.name}</h4>
                        </div>
                      ...
                      <p className="mt-2 line-clamp-2 text-xs leading-5" style={{ color: 'var(--revka-text-secondary)' }}>{integration.description}</p>
                      <div className="mt-2 text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--revka-text-faint)' }}>
          ...
          <div className="flex flex-col gap-4 lg:overflow-y-auto lg:min-h-0">
            <Panel className="p-4" variant="utility">
              <div className="revka-kicker">{t('integrations.selected')}</div>
              {!selectedIntegration ? (
                <div className="mt-4">
                ...
                <>
                  <div className="mt-3 flex items-center gap-2">
                    <RadioTower className="h-4 w-4" style={{ color: 'var(--revka-signal-network)' }} />
                    <div className="text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{selectedIntegration.name}</div>
                  </div>
                  <p className="mt-3 text-sm leading-6" style={{ color: 'var(--revka-text-secondary)' }}>{selectedIntegration.description}</p>
            ...
            <Panel className="p-4" variant="secondary">
              <div className="revka-kicker">{t('integrations.legend')}</div>
...
function DetailRow({
  ...
  return (
    <div className="rounded-[12px] border p-3" style={{ borderColor: 'var(--revka-border-soft)' }}>
      <div className="text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--revka-text-faint)' }}>{label}</div>
      <div className="mt-1 text-sm leading-6" style={{ color: 'var(--revka-text-primary)' }}>{value}</div>
    </div>
  );
}
