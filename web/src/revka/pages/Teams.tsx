  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] flex-col gap-3 md:h-[calc(100vh-6rem)]">
      ...
      <PageHeader
        ...
        actions={
          <>
            <button className="revka-button" data-variant="primary" onClick={() => { setSelectedTeam(null); setEditorOpen(true); }}>
      ...
      >
        <Panel className="flex flex-col overflow-hidden p-0">
          <div className="flex items-center justify-between border-b px-3 py-2" style={{ borderColor: 'var(--revka-border-soft)' }}>
            ...
            <span className="text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{tpl('teams.agent_count', { count: agents.length })}</span>
          </div>
            ...
            {loading ? (
              ...
              teams.map((team) => (
                <button
                  ...
                  style={{
                    borderColor: team.kref === selectedTeam?.kref ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)',
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 text-[11px]" style={{ color: 'var(--revka-text-secondary)' }}>
                        {tpl('teams.members_suffix', { members: team.members.length || team.member_count || 0, edges: team.edges.length || team.edge_count || 0 })}
                      </div>
                    </div>
                    <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ background: team.deprecated ? 'color-mix(in srgb, var(--revka-status-danger) 12%, transparent)' : 'color-mix(in srgb, var(--revka-status-success) 12%, transparent)', color: team.deprecated ? 'var(--revka-status-danger)' : 'var(--revka-status-success)' }}>
            ...
            <>
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{selectedTeam.name}</div>
                    <div className="mt-1">
                      <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{
                        background: selectedTeam.deprecated ? 'color-mix(in srgb, var(--revka-status-danger) 12%, transparent)' : 'color-mix(in srgb, var(--revka-status-success) 12%, transparent)',
                        color: selectedTeam.deprecated ? 'var(--revka-status-danger)' : 'var(--revka-status-success)',
                      }}>
                ...
                <div className="flex items-center gap-1">
                  <button className="revka-button" onClick={() => setEditorOpen(true)} title={t('teams.action.edit')}>
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                  <button className="revka-button" onClick={handleToggleDeprecation} title={selectedTeam.deprecated ? t('teams.action.reenable') : t('teams.action.deprecate')}>
                    <Power className="h-3.5 w-3.5" />
                  </button>
                  <button className="revka-button" onClick={handleDeleteTeam} disabled={deleting} title={t('teams.action.delete')}>
              ...
              <div className="mt-3 grid gap-3 grid-cols-3">
                <Panel className="p-3" variant="utility">
                  <div className="revka-kicker">{t('teams.stats.members')}</div>
                  <div className="mt-1 text-lg font-semibold" style={{ color: 'var(--revka-text-primary)' }}>
                    {selectedTeam.members.length || selectedTeam.member_count || 0}
                  </div>
                </Panel>
                <Panel className="p-3" variant="utility">
                  <div className="revka-kicker">{t('teams.stats.edges')}</div>
                  <div className="mt-1 text-lg font-semibold" style={{ color: 'var(--revka-text-primary)' }}>
                    {selectedTeam.edges.length || selectedTeam.edge_count || 0}
                  </div>
                </Panel>
                <Panel className="p-3" variant="utility">
                  <div className="revka-kicker">{t('teams.stats.status')}</div>
                  <div className="mt-1 text-sm font-semibold" style={{ color: selectedTeam.deprecated ? 'var(--revka-status-danger)' : 'var(--revka-status-success)' }}>
                    {selectedTeam.deprecated ? t('teams.status.deprecated') : t('teams.status.active')}
                  </div>
                </Panel>
              </div>
              ...
              {inspectorTab === 'member' ? (
                <SelectedMemberCard
                  ...
                  footer={selectedMember ? (
                    <Link to="/agents" style={{ color: 'var(--revka-signal-network)' }} className="text-xs">
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-sm">
          <span style={{ color: 'var(--revka-text-secondary)' }}>{t('teams.form.name')}</span>
          <input className="revka-input" value={name} onChange={(event) => setName(event.target.value)} />
        </label>
        <label className="grid gap-2 text-sm">
          <span style={{ color: 'var(--revka-text-secondary)' }}>{t('teams.form.description')}</span>
          <input className="revka-input" value={description} onChange={(event) => setDescription(event.target.value)} />
        </label>
      </div>
      ...
      <div className="mt-4">
        <div className="revka-kicker">{t('teams.form.members')}</div>
        <div className="mt-3 grid max-h-[22rem] gap-2 overflow-auto md:grid-cols-2">
          {agents.map((agent) => {
            const checked = memberKrefs.includes(agent.kref);
            return (
              <label key={agent.kref} className="flex items-start gap-3 rounded-[12px] border p-3" style={{ borderColor: checked ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)' }}>
                <input
                  type="checkbox"
                  checked={checked}
                  onChange={(event) => {
                ...
                <div className="min-w-0">
                  <div className="text-sm font-medium" style={{ color: 'var(--revka-text-primary)' }}>{agent.name}</div>
                  <div className="text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{agent.role} / {agent.agent_type}</div>
                  <div className="mt-1 line-clamp-2 text-xs" style={{ color: 'var(--revka-text-faint)' }}>{agent.identity}</div>
                </div>
              </label>
            );
          })}
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="space-y-4">
          <div>
            <div className="revka-kicker">{t('teams.form.topology_preview')}</div>
            <div className="mt-3">
              <TeamTopologyPanel team={draftTeam} onSelectMember={() => {}} />
            </div>
          </div>
          <Panel className="p-4" variant="utility">
            <div className="revka-kicker">{t('teams.form.edge_authoring')}</div>
            <div className="mt-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_10rem_auto]">
              <select className="revka-input" value={fromKref} onChange={(event) => setFromKref(event.target.value)}>
                <option value="">{t('teams.form.from_member')}</option>
                {selectedAgents.map((agent) => (
                  <option key={agent.kref} value={agent.kref}>{agent.name}</option>
                ))}
              </select>
              <select className="revka-input" value={toKref} onChange={(event) => setToKref(event.target.value)}>
                <option value="">{t('teams.form.to_member')}</option>
                {selectedAgents.map((agent) => (
                  <option key={agent.kref} value={agent.kref}>{agent.name}</option>
                ))}
              </select>
              <select className="revka-input" value={edgeType} onChange={(event) => setEdgeType(event.target.value as TeamEdge['edge_type'])}>
              ...
              <button className="revka-button" onClick={addEdge} disabled={!fromKref || !toKref || fromKref === toKref}>{t('teams.form.add_edge')}</button>
            </div>
            <div className="mt-3 space-y-2">
              {edges.map((edge) => {
                ...
                return (
                  <div key={`${edge.from_kref}-${edge.to_kref}-${edge.edge_type}`} className="flex items-center justify-between gap-2 rounded-[12px] border p-3 text-sm" style={{ borderColor: 'var(--revka-border-soft)' }}>
                    <div style={{ color: 'var(--revka-text-primary)' }}>
                      {fromName} <span style={{ color: 'var(--revka-text-faint)' }}>{edge.edge_type}</span> {toName}
                    </div>
                    <button
                      className="text-xs"
                      style={{ color: 'var(--revka-status-danger)' }}
              ...
              {edges.length === 0 ? (
                <div className="text-sm" style={{ color: 'var(--revka-text-faint)' }}>{t('teams.form.no_edges')}</div>
              ) : null}
            </div>
          </Panel>
        </div>
        <Panel className="p-4" variant="secondary">
          <div className="revka-kicker">{t('teams.form.draft_summary')}</div>
          <div className="mt-3 space-y-2 text-sm">
            <div><span style={{ color: 'var(--revka-text-faint)' }}>{t('teams.form.draft_members')}</span>: <span style={{ color: 'var(--revka-text-primary)' }}>{selectedAgents.length}</span></div>
            <div><span style={{ color: 'var(--revka-text-faint)' }}>{t('teams.form.draft_edges')}</span>: <span style={{ color: 'var(--revka-text-primary)' }}>{edges.length}</span></div>
            <p style={{ color: 'var(--revka-text-secondary)' }}>
      ...
      <div className="mt-5 flex flex-wrap justify-end gap-2">
        <button className="revka-button" onClick={onClose}>{t('teams.form.cancel')}</button>
        <button
