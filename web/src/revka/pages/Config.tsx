...
function colorScalar(v: string): string {
  ...
  if (trimmed === 'true' || trimmed === 'false') return `<span style="color:var(--revka-signal-selected)">${v}</span>`;
  if (/^-?\d[\d_]*(\.[\d_]*)?([eE][+-]?\d+)?$/.test(trimmed)) return `<span style="color:var(--revka-status-warning)">${v}</span>`;
  if (trimmed.startsWith('"') || trimmed.startsWith("'")) return `<span style="color:var(--revka-status-success)">${v}</span>`;
  if (trimmed.startsWith('[') || trimmed.startsWith('{')) return `<span style="color:var(--revka-text-primary)">${v}</span>`;
  return v;
}
...
function highlightToml(raw: string): string {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (/^\s*\[/.test(escaped)) {
      result.push(`<span style="color:var(--revka-signal-network);font-weight:600">${escaped}</span>`);
      continue;
    }
    if (/^\s*#/.test(escaped)) {
    ...
    if (kvMatch) {
      ...
      result.push(`${indent}<span style="color:var(--revka-signal-selected)">${key}</span><span style="color:var(--revka-text-faint)">${eq}</span>${colorValue(rawValue ?? '')}`);
      continue;
    }
    result.push(escaped);
  }
  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] flex-col gap-3 md:h-[calc(100vh-6rem)]">
      ...
      <PageHeader
        ...
        actions={(
          <>
            <button className="revka-button" onClick={() => void loadConfig()}>
              <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />
              {t('config.reload')}
            </button>
            <button className="revka-button" data-variant="primary" onClick={saveConfig} disabled={saving || loading}>
        ...
        <div className="grid gap-4 grid-cols-1 md:min-h-0 md:flex-1 md:grid-cols-[18rem_minmax(0,1fr)] md:[grid-template-rows:minmax(0,1fr)] lg:grid-cols-[18rem_minmax(0,1fr)_24rem]">
          <div className="flex flex-col gap-4 md:overflow-y-auto md:min-h-0">
            <Panel className="shrink-0 p-4" variant="secondary">
              <div className="revka-kicker">{t('config.map')}</div>
              <div className="mt-3 space-y-2">
                {sectionItems.map((section) => (
                  <button
                    key={section.id}
                    type="button"
                    className="revka-selection-card text-left"
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{section.populatedCount}</span>
                    </div>
                    <div className="mt-2 text-xs leading-5" style={{ color: 'var(--revka-text-secondary)' }}>{section.description}</div>
                  </button>
                ))}
              </div>
            </Panel>
          ...
          <Panel className="flex min-h-0 flex-col p-5">
            <div className="flex shrink-0 items-center justify-between gap-3">
              <div>
                <div className="revka-kicker">{t('config.workspace')}</div>
                ...
                {workspaceTab === 'structured' && activeSectionItem ? (
                  <p className="mt-2 max-w-2xl text-sm leading-6" style={{ color: 'var(--revka-text-secondary)' }}>
                    {activeSectionItem.description}
                  </p>
                ) : null}
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <div className="revka-tab-strip" role="tablist" aria-label={t('config.tabs_aria')}>
                  <button
                    type="button"
                ...
                <button className="revka-button" onClick={() => setShowPreview((current) => !current)}>
            ...
            {workspaceTab === 'structured' ? (
                ...
                <div className="mt-5 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
                  ...
                  {activeSectionItem?.paths?.length ? (
                    <Panel className="p-4" variant="secondary">
                      <div className="revka-kicker">{t('config.mapped_paths')}</div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {activeSectionItem.paths.map((path) => (
                          <span key={path} className="rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ borderColor: 'var(--revka-border-soft)', color: 'var(--revka-text-secondary)' }}>
                            {path}
                          </span>
                        ))}
                      </div>
              ...
              <div className={`mt-5 grid min-h-0 flex-1 gap-4 ${showPreview ? 'xl:grid-cols-2' : 'grid-cols-1'}`}>
                <textarea
                  ...
                  className="revka-input h-full min-h-0 w-full resize-none"
                  style={{ fontFamily: 'var(--pc-font-mono)', lineHeight: 1.65 }}
                  spellCheck={false}
                />

                {showPreview ? (
                  <div className="min-h-0 overflow-auto rounded-[16px] border p-4" style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-base) 82%, var(--revka-bg-panel-strong))' }}>
                    <pre
                      className="whitespace-pre-wrap break-words text-xs leading-7"
                      style={{ color: 'var(--revka-text-secondary)', fontFamily: 'var(--pc-font-mono)' }}
          ...
          <div className="flex flex-col gap-4 lg:min-h-0 lg:overflow-hidden">
            <Panel className="flex min-h-0 flex-1 flex-col p-4" variant="secondary">
              <div className="inline-flex shrink-0 items-center gap-2">
                <Settings className="h-4 w-4" style={{ color: 'var(--revka-signal-network)' }} />
                <div className="revka-kicker">{t('config.summary.title')}</div>
              </div>
              {parsedConfig ? (
            ...
            <Panel className="flex max-h-[22rem] shrink-0 flex-col p-4" variant="utility">
              <div className="revka-kicker shrink-0">{t('config.source_health')}</div>
      ...
      return (
        <ConfigSectionCard title={t('config.section.provider')}>
          <EditableField label={t('config.provider.default_provider')}>
            <input className="revka-input" value={parsedConfig.default_provider ?? ''} onChange={(event) => updateField(null, 'default_provider', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.provider.default_model')}>
            <input className="revka-input" value={parsedConfig.default_model ?? ''} onChange={(event) => updateField(null, 'default_model', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.provider.temperature')}>
            <input className="revka-input" type="number" step="0.1" value={parsedConfig.default_temperature ?? 0.7} onChange={(event) => updateField(null, 'default_temperature', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.provider.timeout')}>
            <input className="revka-input" type="number" value={parsedConfig.provider_timeout_secs ?? 120} onChange={(event) => updateField(null, 'provider_timeout_secs', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.provider.max_tokens')}>
            <input className="revka-input" type="number" value={parsedConfig.provider_max_tokens ?? 8192} onChange={(event) => updateField(null, 'provider_max_tokens', Number(event.target.value))} />
          </EditableField>
        </ConfigSectionCard>
      );
    case 'agent':
      return (
        <ConfigSectionCard title={t('config.section.agent')}>
          <EditableField label={t('config.agent.max_tool_iterations')}>
            <input className="revka-input" type="number" value={parsedConfig.agent?.max_tool_iterations ?? 10} onChange={(event) => updateField('agent', 'max_tool_iterations', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.agent.max_context_tokens')}>
            <input className="revka-input" type="number" value={parsedConfig.agent?.max_context_tokens ?? 32000} onChange={(event) => updateField('agent', 'max_context_tokens', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.agent.max_history_messages')}>
            <input className="revka-input" type="number" value={parsedConfig.agent?.max_history_messages ?? 40} onChange={(event) => updateField('agent', 'max_history_messages', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.agent.gpt55_context_window')}>
            <input className="revka-input" type="number" min={1} value={parsedConfig.agent?.model_context_windows?.['gpt-5_5'] ?? 1050000} onChange={(event) => updateField('agent.model_context_windows', 'gpt-5_5', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.agent.context_window_safety_ratio')}>
            <input className="revka-input" type="number" min={0.1} max={1} step={0.01} value={parsedConfig.agent?.context_window_safety_ratio ?? 0.95} onChange={(event) => updateField('agent', 'context_window_safety_ratio', Number(event.target.value))} />
          </EditableField>
          ...
          <EditableField label={t('config.agent.thinking_level')}>
            <input className="revka-input" value={parsedConfig.agent?.thinking?.default_level ?? 'medium'} onChange={(event) => updateField('agent.thinking', 'default_level', event.target.value)} />
          </EditableField>
        </ConfigSectionCard>
      );
    case 'memory':
      return (
        <ConfigSectionCard title={t('config.section.memory')}>
          ...
          <EditableField label={t('config.memory.memory_project')}>
            <input className="revka-input" value={parsedConfig.kumiho?.memory_project ?? ''} onChange={(event) => updateField('kumiho', 'memory_project', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.memory.harness_project')}>
            <input className="revka-input" value={parsedConfig.kumiho?.harness_project ?? ''} onChange={(event) => updateField('kumiho', 'harness_project', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.memory.retrieval_limit')}>
            <input className="revka-input" type="number" min={1} max={50} value={parsedConfig.kumiho?.memory_retrieval_limit ?? 3} onChange={(event) => updateField('kumiho', 'memory_retrieval_limit', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.memory.backend')}>
            <input className="revka-input" value={parsedConfig.memory?.backend ?? ''} onChange={(event) => updateField('memory', 'backend', event.target.value)} />
          </EditableField>
      ...
      return (
        <ConfigSectionCard title={t('config.section.runtime')}>
          <EditableField label={t('config.runtime.gateway_host')}>
            <input className="revka-input" value={parsedConfig.gateway?.host ?? ''} onChange={(event) => updateField('gateway', 'host', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.runtime.gateway_port')}>
            <input className="revka-input" type="number" value={parsedConfig.gateway?.port ?? 42617} onChange={(event) => updateField('gateway', 'port', Number(event.target.value))} />
          </EditableField>
          ...
          <EditableField label={t('config.runtime.autonomy_level')}>
            <input className="revka-input" value={parsedConfig.autonomy?.level ?? ''} onChange={(event) => updateField('autonomy', 'level', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.runtime.max_actions_hour')}>
            <input className="revka-input" type="number" value={parsedConfig.autonomy?.max_actions_per_hour ?? 0} onChange={(event) => updateField('autonomy', 'max_actions_per_hour', Number(event.target.value))} />
          </EditableField>
        </ConfigSectionCard>
      );
    case 'security':
      return (
        <ConfigSectionCard title={t('config.section.security')}>
          ...
          <EditableField label={t('config.security.sandbox_backend')}>
            <input className="revka-input" value={parsedConfig.security?.sandbox?.backend ?? ''} onChange={(event) => updateField('security.sandbox', 'backend', event.target.value)} />
          </EditableField>
          <EditableField label={t('config.security.daily_limit')}>
            <input className="revka-input" type="number" step="0.01" value={parsedConfig.cost?.daily_limit_usd ?? 0} onChange={(event) => updateField('cost', 'daily_limit_usd', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.security.monthly_limit')}>
            <input className="revka-input" type="number" step="0.01" value={parsedConfig.cost?.monthly_limit_usd ?? 0} onChange={(event) => updateField('cost', 'monthly_limit_usd', Number(event.target.value))} />
          </EditableField>
          <EditableField label={t('config.security.warn_percent')}>
            <input className="revka-input" type="number" value={parsedConfig.cost?.warn_at_percent ?? 80} onChange={(event) => updateField('cost', 'warn_at_percent', Number(event.target.value))} />
          </EditableField>
        </ConfigSectionCard>
      );
    case 'channels':
      return (
        <ConfigSectionCard title={t('config.section.channels')}>
          ...
          <EditableField label={t('config.channels.message_timeout')}>
            <input className="revka-input" type="number" value={parsedConfig.channels_config?.message_timeout_secs ?? 60} onChange={(event) => updateField('channels_config', 'message_timeout_secs', Number(event.target.value))} />
          </EditableField>
        </ConfigSectionCard>
      );
    case 'mcp':
...
function EditableField({
  ...
  return (
    <label className="grid gap-2 text-sm">
      <span style={{ color: 'var(--revka-text-secondary)' }}>{label}</span>
      {children}
    </label>
  );
}
...
function EditableToggle({
  ...
  return (
    <button
      ...
      style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 88%, transparent)' }}
      onClick={() => onChange(!checked)}
    >
      <span className="text-sm" style={{ color: 'var(--revka-text-primary)' }}>{label}</span>
      <span
        className="rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]"
        style={{
          background: checked ? 'var(--revka-signal-selected-soft, color-mix(in srgb, var(--revka-signal-selected) 18%, transparent))' : 'color-mix(in srgb, var(--revka-text-faint) 14%, transparent)',
          color: checked ? 'var(--revka-signal-selected)' : 'var(--revka-text-faint)',
        }}
      >
...
function ConfigRow({
  ...
  const { t } = useT();
  return (
    <div className="rounded-[12px] border p-3" style={{ borderColor: 'var(--revka-border-soft)' }}>
      <div className="text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--revka-text-faint)' }}>
        {label}
      </div>
      <div className="mt-1 text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>
    ...
    <div className="space-y-4">
      ...
      <Panel className="p-4" variant="utility">
        <div className="flex items-center justify-between">
          <div>
            <div className="revka-kicker">{t('config.mcp.servers')}</div>
            <p className="mt-1 text-xs" style={{ color: 'var(--revka-text-secondary)' }}>
              {serverCountMessage}
            </p>
          </div>
          <button type="button" className="revka-button" data-variant="primary" onClick={addServer}>
...
function McpServerCard({
  ...
  // can iterate on a config before committing it to revka.toml.
  const runTest = async () => {
  ...
  return (
    <div
      ...
      style={{
        borderColor: hasError ? 'var(--revka-status-error, var(--revka-status-danger))' : 'var(--revka-border-soft)',
        background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 88%, transparent)',
      }}
    >
      <div className="flex items-center justify-between px-4 py-3">
        ...
        >
          {expanded ? (
            <ChevronDown className="h-4 w-4" style={{ color: 'var(--revka-text-secondary)' }} />
          ) : (
            <ChevronRight className="h-4 w-4" style={{ color: 'var(--revka-text-secondary)' }} />
          )}
          <div className="min-w-0">
            <div className="text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{title}</div>
            <div className="truncate text-xs" style={{ color: 'var(--revka-text-secondary)' }}>
      ...
      {expanded ? (
        <div className="space-y-4 border-t px-4 py-4" style={{ borderColor: 'var(--revka-border-soft)' }}>
          <EditableField label={t('config.mcp.field_name')}>
            <input
              className="revka-input"
              value={server.name}
...
function TestResultBadge({ state, t, tpl }: { state: TestState; t: TFn; tpl: TplFn }) {
  ...
  if (result.ok) {
    ...
    return (
      <span
        ...
        style={{
          borderColor: 'var(--revka-status-ok, var(--revka-status-success))',
          color: 'var(--revka-status-ok, var(--revka-status-success))',
        }}
        title={
  ...
  return (
    <span
      ...
      style={{
        borderColor: 'var(--revka-status-error, var(--revka-status-danger))',
        color: 'var(--revka-status-error, var(--revka-status-danger))',
      }}
...
function FieldError({ message }: { message: string }) {
  return (
    <span className="text-xs" style={{ color: 'var(--revka-status-error, #f87171)' }}>
      {message}
    </span>
  );
}
  ...
  return (
    <div className="grid gap-2 text-sm">
      ...
      <div className="space-y-2">
        ...
        <button type="button" className="revka-button" onClick={add}>
          <Plus className="h-4 w-4" />
    ...
    return (
      <EditableField label={fieldKey}>
        <div
          ...
          style={{ borderColor: 'var(--revka-border-soft)', color: 'var(--revka-text-secondary)' }}
        >
  ...
  if (value !== null && typeof value === 'object') {
    return (
      <Panel className="p-3" variant="utility">
        <div className="revka-kicker">{`${pathPrefix}.${fieldKey}`}</div>
