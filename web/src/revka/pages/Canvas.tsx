...
export default function Canvas() {
  ...
  const connectWs = useCallback((id: string) => {
    ...
    const token = getToken();
    const protocols = token ? ['revka.v1', `bearer.${token}`] : ['revka.v1'];
  ...
  const iframePalette = useMemo(() => {
    ...
    return {
      bg: styles.getPropertyValue('--revka-bg-base').trim() || (resolvedTheme === 'light' ? '#f9f2f4' : '#070607'),
      text: styles.getPropertyValue('--revka-text-primary').trim() || (resolvedTheme === 'light' ? '#261019' : '#fff3f5'),
    };
  }, [resolvedTheme]);
  ...
  const viewportBackground = resolvedTheme === 'light'
    ? 'linear-gradient(180deg, color-mix(in srgb, var(--revka-bg-surface) 74%, transparent), var(--revka-bg-base))'
    : 'linear-gradient(180deg, color-mix(in srgb, var(--revka-bg-surface) 92%, transparent), var(--revka-bg-base))';

  const viewportGrid = 'var(--revka-grid-line)';
  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] flex-col gap-3 lg:h-[calc(100vh-6rem)]">
      <PageHeader
        ...
        actions={(
          <button className="revka-button" onClick={() => connectWs(canvasId)}>
      ...
      <div className="grid gap-4 grid-cols-1 lg:min-h-0 lg:flex-1 lg:grid-cols-[20rem_minmax(0,1fr)_18rem]">
        <div className="flex flex-col gap-4 lg:overflow-y-auto lg:min-h-0">
          <Panel className="p-4">
            <div className="revka-kicker">{t('canvas.control')}</div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span
                className="revka-status-pill"
                style={{
                  color: connected ? 'var(--revka-signal-selected)' : 'var(--revka-status-danger)',
                  background: connected ? 'color-mix(in srgb, var(--revka-signal-selected) 12%, transparent)' : 'color-mix(in srgb, var(--revka-status-danger) 12%, transparent)',
              ...
              <span className="revka-status-pill" style={{ color: 'var(--revka-text-secondary)' }}>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <button className="revka-button" data-variant="primary" onClick={handleSwitchCanvas}>
                  {t('canvas.switch')}
                </button>
                <button className="revka-button" onClick={handleClear}>
          ...
          <Panel className="p-4" variant="secondary">
            <div className="revka-kicker">{t('canvas.available')}</div>
              ...
              {loadingList ? (
                ...
                canvasList.map((id) => (
                  <button
                    ...
                    className="revka-selection-card"
                    onClick={() => {
                      setCanvasIdInput(id);
                      setCanvasId(id);
                      setCurrentFrame(null);
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="truncate text-sm font-medium" style={{ color: 'var(--revka-text-primary)' }}>{id}</span>
                      {id === canvasId ? (
                        <span className="text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--revka-signal-selected)' }}>
                          {t('canvas.active')}
                        </span>
                      ) : null}
                    </div>
        ...
        <Panel className="p-5 lg:overflow-y-auto">
          <div className="revka-kicker">{t('canvas.viewport')}</div>
          <h3 className="mt-2 text-lg font-semibold">{t('canvas.live_output')}</h3>

          <div className="mt-4 grid gap-3 md:grid-cols-4">
            <Panel className="p-3" variant="utility">
              <div className="revka-kicker">{t('canvas.canvas_label')}</div>
              <div className="mt-2 text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{canvasId}</div>
            </Panel>
            <Panel className="p-3" variant="utility">
              <div className="revka-kicker">{t('canvas.connection')}</div>
              <div className="mt-2 text-sm font-semibold" style={{ color: connected ? 'var(--revka-signal-selected)' : 'var(--revka-status-danger)' }}>
                {connected ? t('canvas.live') : t('canvas.offline')}
              </div>
            </Panel>
            <Panel className="p-3" variant="utility">
              <div className="revka-kicker">{t('canvas.frame_type')}</div>
              <div className="mt-2 text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>
                {currentFrame?.content_type ?? '--'}
              </div>
            </Panel>
            <Panel className="p-3" variant="utility">
              <div className="revka-kicker">{t('canvas.last_update')}</div>
              <div className="mt-2 text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>
                {currentFrameTime}
              </div>
            </Panel>
          ...
          >
            ...
            <div
              ...
              style={{ borderColor: 'var(--revka-border-soft)', position: 'relative', zIndex: 1, background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 86%, transparent)' }}
            >
              <div className="flex items-center gap-2">
                <span className="h-2.5 w-2.5 rounded-full" style={{ background: 'var(--revka-status-danger)' }} />
                <span className="h-2.5 w-2.5 rounded-full" style={{ background: 'var(--revka-status-warning)' }} />
                <span className="h-2.5 w-2.5 rounded-full" style={{ background: 'var(--revka-status-success)' }} />
              </div>
              <div className="text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--revka-text-faint)' }}>
          ...
          {currentFrame ? (
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <Panel className="p-3" variant="utility">
                <div className="revka-kicker">{t('canvas.frame_id')}</div>
                <div className="mt-2 text-sm font-mono" style={{ color: 'var(--revka-text-primary)' }}>
                  {currentFrame.frame_id}
                </div>
              </Panel>
              <Panel className="p-3" variant="utility">
                <div className="revka-kicker">{t('canvas.guidance')}</div>
                <div className="mt-2 text-sm leading-6" style={{ color: 'var(--revka-text-secondary)' }}>
        ...
        <Panel className="p-4 lg:overflow-y-auto" variant="utility">
          <div className="flex items-center justify-between gap-2">
            <div className="revka-kicker">{t('canvas.history')}</div>
            <button className="revka-button" onClick={() => setShowHistory((current) => !current)}>
          ...
          <div className="mt-3 space-y-2">
            {!showHistory ? (
              ...
              [...history].reverse().map((frame) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-xs font-semibold uppercase tracking-[0.12em]" style={{ color: 'var(--revka-signal-network)' }}>
                      {frame.content_type}
                    </span>
