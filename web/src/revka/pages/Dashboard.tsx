...
export default function Dashboard() {
  ...
  const dashboardSceneStyle = useMemo(
    () => dashboardScene
      ? ({ '--revka-dashboard-scene-image': `url("${dashboardScene.replace(/"/g, '%22')}")` } as CSSProperties)
      : undefined,
    [dashboardScene],
  );
  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] flex-col gap-3 lg:h-[calc(100vh-6rem)]">
      ...
      {dashboardScene ? (
        <Panel className="revka-dashboard-showcase overflow-hidden p-0" variant="utility">
          <div className="revka-dashboard-scene" style={dashboardSceneStyle}>
      ...
      <div className="grid gap-4 grid-cols-1 lg:min-h-0 lg:flex-1 lg:grid-cols-[minmax(0,1fr)_24rem] lg:[grid-template-rows:minmax(0,1fr)]">
        <Panel className="flex flex-col p-5 lg:min-h-0">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="revka-kicker">{t('dashboard.live_orchestration_pane')}</div>
              <h3 className="mt-2 text-lg font-semibold">
                {selectedRun ? `${selectedRun.workflow_name} / ${selectedRun.run_id.slice(0, 8)}` : t('dashboard.active_workflow_posture')}
              </h3>
            </div>
            <Workflow className="h-5 w-5" style={{ color: 'var(--revka-signal-network)' }} />
          </div>
          ...
          <div ref={workspaceRef} tabIndex={-1} className="mt-6 grid gap-4 outline-none lg:min-h-0 lg:flex-1 lg:grid-cols-[minmax(0,1fr)_22rem] lg:[grid-template-rows:minmax(0,1fr)]">
                ...
                <div className="space-y-2">
                  <OperatorSection title={t('dashboard.run_posture')}>
                    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
                      <OperatorCountChip label={t('dashboard.count.running')} value={runStepCounts.running} tone="var(--revka-signal-live)" compact assetSlot="statusRunningBadge" />
                      <OperatorCountChip label={t('dashboard.count.failed')} value={runStepCounts.failed} tone="var(--revka-status-danger)" compact assetSlot="statusFailedBadge" />
                      <OperatorCountChip label={t('dashboard.count.completed')} value={runStepCounts.completed} tone="var(--revka-status-success)" compact assetSlot="statusSuccessBadge" />
                      <OperatorCountChip label={t('dashboard.count.pending')} value={runStepCounts.pending} tone="var(--revka-text-faint)" compact assetSlot="statusPendingBadge" />
                      <OperatorCountChip label={t('dashboard.count.skipped')} value={runStepCounts.skipped} tone="var(--revka-status-idle)" compact assetSlot="statusSkippedBadge" />
                    </div>
                  </OperatorSection>
            ...
            <div className="space-y-4 lg:min-h-0 lg:overflow-y-auto lg:pr-1">
              ...
              <Panel className="p-4" variant="utility" skinSlot="timeline">
                <div className="revka-kicker">{t('dashboard.priority_timeline')}</div>
                <div className="mt-3 space-y-2">
                  {selectedRun?.steps.map((step) => (
                    <button
                      ...
                      type="button"
                      onClick={() => focusTaskById(step.step_id)}
                      className="block w-full rounded-[8px] border p-3 text-left"
                      style={{ borderColor: selectedTask?.id === step.step_id ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)' }}
                    >
                  ...
                  )) ?? <div className="text-sm" style={{ color: 'var(--revka-text-faint)' }}>{t('dashboard.no_run_selected')}</div>}
                </div>
              </Panel>
            </div>
          </div>
        ...
        {/* Right rail — single column, 4 cards, reading order is the
            ...
            `.revka-panel { overflow: hidden }` (from theme.css),
        ...
        <div className="flex flex-col gap-4 lg:max-h-[calc(100vh-9rem)] lg:min-h-0 lg:overflow-y-auto lg:pr-1 [&>*]:shrink-0">
          ...
          <RecentRunsRailCard
            ...
            footer={(
              <Link to="/runs" className="inline-flex items-center gap-2 text-sm" style={{ color: 'var(--revka-signal-network)' }}>
