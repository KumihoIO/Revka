...
export default function Skills() {
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] flex-col gap-3">
      ...
      <div className="revka-skills-grid grid min-h-0 flex-1 gap-4">
        <div className="flex min-h-0 flex-col gap-4">
          <Panel className="p-4" variant="secondary">
            <div className="revka-kicker">{t('skills.registry_status')}</div>
          ...
          <Panel className="flex min-h-0 flex-1 flex-col p-4" variant="secondary">
            <div className="flex items-center justify-between gap-2">
              <div className="revka-kicker">{t('skills.index')}</div>
                ...
                filteredSkills.map((skill) => (
                  <button
                    ...
                    style={{
                      ...
                      borderColor: skill.kref === selectedSkillKref ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)',
                      background: skill.kref === selectedSkillKref
                        ? 'color-mix(in srgb, var(--revka-signal-selected) 14%, var(--revka-bg-panel-strong))'
                        : 'color-mix(in srgb, var(--revka-bg-panel-strong) 94%, transparent)',
                    }}
                  >
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px]" style={{ background: 'color-mix(in srgb, var(--revka-signal-network) 12%, transparent)', color: 'var(--revka-signal-network)' }}>
                        <Sparkles className="h-4 w-4" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <div className="text-sm font-semibold leading-5" style={{ color: 'var(--revka-text-primary)' }}>{skill.name}</div>
                          <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ background: skill.deprecated ? 'color-mix(in srgb, var(--revka-status-warning) 12%, transparent)' : 'color-mix(in srgb, var(--revka-signal-selected) 12%, transparent)', color: skill.deprecated ? 'var(--revka-status-warning)' : 'var(--revka-signal-selected)' }}>
                            {skill.deprecated ? t('skills.status.deprecated') : t('skills.status.active')}
                          </span>
                        </div>
                        <div className="mt-1 text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{skill.domain}</div>
                        <div className="mt-1.5 line-clamp-2 text-[11px] leading-4" style={{ color: 'var(--revka-text-faint)' }}>{skill.description}</div>
                      </div>
                    </div>
                  </button>
                ))
        ...
        <Panel className="overflow-y-auto p-5">
          <div className="revka-kicker">{t('skills.workspace')}</div>
          ...
          {selectedSkill ? (
            <div className="mt-4 space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <Sparkles className="h-4 w-4" style={{ color: 'var(--revka-signal-network)' }} />
                    <div className="text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{selectedSkill.name}</div>
                  </div>
                ...
                <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{
                  background: selectedSkill.deprecated ? 'color-mix(in srgb, var(--revka-status-warning) 12%, transparent)' : 'color-mix(in srgb, var(--revka-signal-selected) 12%, transparent)',
                  color: selectedSkill.deprecated ? 'var(--revka-status-warning)' : 'var(--revka-signal-selected)',
                }}>
              ...
              <div className="flex flex-wrap gap-2">
                <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ background: 'var(--revka-signal-selected-soft, color-mix(in srgb, var(--revka-signal-selected) 18%, transparent))', color: 'var(--revka-signal-selected)' }}>
                  {selectedSkill.domain}
                </span>
                {selectedSkill.tags.map((tag) => (
                  <span key={tag} className="rounded-full border px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ borderColor: 'var(--revka-border-soft)', color: 'var(--revka-text-secondary)' }}>
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <Panel className="p-4" variant="utility">
                <div className="revka-kicker">{t('skills.description')}</div>
                <div className="mt-3 text-sm leading-6" style={{ color: 'var(--revka-text-secondary)' }}>
                  {selectedSkill.description}
                </div>
              </Panel>

              <div>
                <div className="revka-kicker">{t('skills.content')}</div>
                {skillDetailLoading ? (
                  <div className="mt-2 rounded-[14px] border p-4" style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 94%, transparent)' }}>
                    <StateMessage compact tone="loading" title={t('skills.loading')} />
                  </div>
                ) : skillDetailError ? (
                  <div className="mt-2 rounded-[14px] border p-4" style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 94%, transparent)' }}>
              ...
              <div className="flex gap-2">
                <button className="revka-button" onClick={() => setEditorMode('edit')} disabled={skillDetailLoading || !selectedSkillDetail}>
                ...
                <button className="revka-button" onClick={handleDelete} disabled={deleting}>
        ...
        <div className="flex min-h-0 flex-col gap-4">
          <Panel className="p-4" variant="utility">
            <div className="revka-kicker">{t('skills.install_github')}</div>
            <p className="mt-2 text-xs leading-5" style={{ color: 'var(--revka-text-secondary)' }}>
              {t('skills.install_github_desc')}
            </p>
            <div className="mt-3 flex gap-2">
              <div className="relative min-w-0 flex-1">
                <ExternalLink className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: 'var(--revka-text-faint)' }} />
                <input
                  className="revka-input pl-10"
                  value={githubUrl}
              ...
              <button
                className="revka-button shrink-0"
          ...
          <Panel className="flex min-h-0 flex-1 flex-col p-4" variant="utility">
            <div className="flex items-center justify-between gap-3">
              <div className="revka-kicker">{t('skills.marketplace')}</div>
              <div className="text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{tpl('skills.marketplace_shown', { count: marketplaceResults.length })}</div>
            </div>
            <div className="relative mt-3">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: 'var(--revka-text-faint)' }} />
              <input
                className="revka-input pl-10"
            ...
            <div className="mt-3 flex gap-2">
              <button className="revka-button" onClick={() => loadMarketplace(marketplaceQuery)}>{t('skills.search')}</button>
              <button className="revka-button" onClick={() => { setMarketplaceQuery(''); loadMarketplace(); }}>{t('skills.trending')}</button>
            </div>
            <div className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
              {marketplaceLoading ? (
                ...
                marketplaceResults.map((skill) => (
                  <div key={skill.slug} className="rounded-[12px] border p-3" style={{ borderColor: 'var(--revka-border-soft)' }}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{skill.displayName || skill.name || skill.slug}</div>
                        <div className="mt-1 line-clamp-2 text-xs" style={{ color: 'var(--revka-text-secondary)' }}>{skill.description}</div>
                      </div>
                      <button className="revka-button shrink-0" onClick={() => handleInstall(skill.slug)}>{t('skills.install')}</button>
                    </div>
                  </div>
                ))
              )}
...
function SummaryMetric({
  ...
  return (
    <div className="rounded-[14px] border p-3" style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 88%, transparent)' }}>
      ...
      <div className="mt-2 text-xl font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{value}</div>
    </div>
  );
}
  ...
  return (
    <Modal title={skill ? t('skills.modal.edit_title') : t('skills.modal.create_title')} description={t('skills.modal.description')} onClose={onClose}>
      <div className="grid gap-4">
        <input className="revka-input" value={name} onChange={(event) => setName(event.target.value)} placeholder={t('skills.modal.name_placeholder')} />
        <textarea className="revka-input min-h-24" value={description} onChange={(event) => setDescription(event.target.value)} placeholder={t('skills.modal.description_placeholder')} />
        <select className="revka-input" value={domain} onChange={(event) => setDomain(event.target.value)}>
          {DOMAINS.map((option) => <option key={option} value={option}>{option}</option>)}
        </select>
        <input className="revka-input" value={tags} onChange={(event) => setTags(event.target.value)} placeholder={t('skills.modal.tags_placeholder')} />
        <textarea className="revka-input min-h-72" value={content} onChange={(event) => setContent(event.target.value)} placeholder={t('skills.modal.content_placeholder')} style={{ fontFamily: 'var(--pc-font-mono)' }} />
        <div className="flex justify-end gap-3">
          <button className="revka-button" onClick={onClose}>{t('skills.modal.cancel')}</button>
          <button
