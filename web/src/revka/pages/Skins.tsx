      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="revka-kicker">{skin.id}</div>
            <h3 className="mt-1 truncate text-lg font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{skin.name}</h3>
            <p className="mt-1 text-xs" style={{ color: 'var(--revka-text-faint)' }}>
              v{skin.version}{skin.builtin ? ' · Built-in' : ''}
            </p>
          </div>
          {active ? (
            <span className="revka-status-pill" style={{ color: 'var(--revka-signal-live)' }}>
        ...
        {error ? (
          <div className="rounded-[10px] border px-3 py-2 text-sm" style={{ borderColor: 'var(--revka-status-danger)', color: 'var(--revka-status-danger)' }}>
            {error}
          </div>
        ) : null}

        <div className="flex flex-wrap gap-2">
          <button type="button" className="revka-button text-sm" onClick={() => setSkin(active ? null : skin.id)} disabled={busy}>
            {active ? <EyeOff className="h-4 w-4" /> : <Check className="h-4 w-4" />}
            {active ? 'Deactivate' : 'Activate'}
          </button>
          {skin.builtin ? null : (
            <button type="button" className="revka-button text-sm" onClick={onDelete} disabled={busy}>
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        title="Skins"
        actions={(
          <button type="button" className="revka-button text-sm" onClick={() => void refreshSkins()} disabled={skinsLoading}>
      ...
      <Panel className="p-5">
        <label
          ...
          style={{
            borderColor: dragging ? 'var(--revka-border-strong)' : 'var(--revka-border-soft)',
            background: dragging ? 'var(--revka-signal-live-soft)' : 'var(--revka-bg-surface)',
          }}
          onDragEnter={(event) => {
            event.preventDefault();
            setDragging(true);
          ...
          onDrop={onDrop}
        >
          <UploadCloud className="h-7 w-7" style={{ color: 'var(--revka-signal-network)' }} />
          <span className="mt-3 text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{importing ? 'Importing ZIP...' : 'Upload skin ZIP'}</span>
          <span className="mt-1 text-xs" style={{ color: 'var(--revka-text-faint)' }}>revka-skin.json, tokens, and local image assets</span>
          <input
            className="sr-only"
            type="file"
        ...
        {error ? (
          <div className="mt-4 rounded-[10px] border px-3 py-2 text-sm" style={{ borderColor: 'var(--revka-status-danger)', color: 'var(--revka-status-danger)' }} aria-live="polite">
            {error}
          </div>
        ) : null}
      </Panel>
      ...
      {!skinsLoading && installedSkins.length === 0 ? (
        <Panel className="p-5" variant="utility">
          <div className="text-sm" style={{ color: 'var(--revka-text-secondary)' }}>No skins installed.</div>
        </Panel>
      ) : null}
    </div>
  );
