  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] flex-col gap-3">
      ...
      <PageHeader
        ...
        actions={(
          <button className="revka-button" onClick={load} disabled={loading}>
      ...
      <Panel className="p-3" variant="secondary">
        <div className="relative">
          ...
          <input
            type="text"
            className="revka-input w-full pl-9"
            value={search}
      ...
      {loading ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4">
          <Panel className={`flex flex-col p-4 ${agentSectionOpen ? 'min-h-0 flex-1' : ''}`}>
            ...
            >
              <Wrench className="h-4 w-4" style={{ color: 'var(--revka-signal-network)' }} />
              <span className="revka-kicker flex-1">{tpl('tools.agent_tools_count', { count: filteredAgent.length })}</span>
              <ChevronDown
                className="h-4 w-4"
                style={{
                  ...
                  {filteredAgent.map((tool) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 70%, transparent)' }}
                      >
                        <button
                          type="button"
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex min-w-0 items-center gap-2">
                              <Package className="h-4 w-4 shrink-0" style={{ color: 'var(--revka-signal-network)' }} />
                              <h3 className="truncate text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{tool.name}</h3>
                            </div>
                            {isExpanded
                              ? <ChevronDown className="h-4 w-4 shrink-0" style={{ color: 'var(--revka-signal-network)' }} />
                              : <ChevronRight className="h-4 w-4 shrink-0" style={{ color: 'var(--revka-text-faint)' }} />}
                          </div>
                          <p className="mt-2 line-clamp-2 text-sm" style={{ color: 'var(--revka-text-secondary)' }}>
                            {tool.description}
                          </p>
                        </button>
                        {isExpanded && tool.parameters ? (
                          <div className="border-t p-3" style={{ borderColor: 'var(--revka-border-soft)' }}>
                            <div className="revka-kicker mb-2">{t('tools.parameter_schema')}</div>
                            <pre
          ...
          <Panel className={`flex flex-col p-4 ${cliSectionOpen ? 'min-h-0 flex-1' : ''}`}>
            ...
            >
              <Terminal className="h-4 w-4" style={{ color: 'var(--revka-signal-selected)' }} />
              <span className="revka-kicker flex-1">{tpl('tools.cli_tools_count', { count: filteredCli.length })}</span>
              <ChevronDown
                className="h-4 w-4"
                style={{
                  color: 'var(--revka-text-faint)',
                  ...
                  <table className="revka-table w-full">
                    ...
                    <tbody>
                      {filteredCli.map((tool) => (
                        <tr key={tool.name}>
                          <td className="text-sm font-medium" style={{ color: 'var(--revka-text-primary)' }}>{tool.name}</td>
                          <td className="max-w-[260px] truncate font-mono text-xs" title={tool.path} style={{ color: 'var(--revka-text-faint)' }}>
                            {tool.path}
                          </td>
                          <td style={{ color: 'var(--revka-text-faint)' }}>{tool.version ?? '—'}</td>
                          <td>
                            <span
                              ...
                              style={{
                                ...
                                background: 'color-mix(in srgb, var(--revka-signal-selected) 8%, transparent)',
                              }}
                            >
                              {tool.category}
                            </span>
