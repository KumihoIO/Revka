        ...
        <div className="grid gap-4 grid-cols-1 md:min-h-0 md:flex-1 md:grid-cols-[18rem_minmax(0,1fr)] lg:grid-cols-[18rem_minmax(0,1fr)_22rem]">
          <div className="flex flex-col gap-4 md:overflow-y-auto md:min-h-0">
            <Panel className="p-4" variant="secondary">
              <div className="revka-kicker">{t('cost.spend_posture')}</div>
              <div className="mt-4 space-y-3">
                {[
                  { icon: DollarSign, tone: 'var(--revka-signal-live)', label: t('cost.session_cost'), value: formatUSD(cost.session_cost_usd) },
                  { icon: TrendingUp, tone: 'var(--revka-status-success)', label: t('cost.daily_cost'), value: formatUSD(cost.daily_cost_usd) },
                  { icon: Layers, tone: 'var(--revka-signal-selected)', label: t('cost.monthly_cost'), value: formatUSD(cost.monthly_cost_usd) },
                  { icon: Hash, tone: 'var(--revka-status-warning)', label: t('cost.requests'), value: cost.request_count.toLocaleString() },
                ].map(({ icon: Icon, tone, label, value }) => (
                  <div key={label} className="rounded-[14px] border p-3" style={{ borderColor: 'var(--revka-border-soft)', background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 88%, transparent)' }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 font-mono text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{value}</div>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <Panel className="p-4" variant="secondary">
              <div className="revka-kicker">{t('cost.token_efficiency')}</div>
          ...
          <Panel className="p-5 md:overflow-y-auto">
            <div className="revka-kicker">{t('cost.model_workspace')}</div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{model.model}</div>
                        <div className="font-mono text-sm" style={{ color: 'var(--revka-text-secondary)' }}>{formatUSD(model.cost_usd)}</div>
                      </div>
                      <div className="mt-2 grid gap-2 text-xs md:grid-cols-3" style={{ color: 'var(--revka-text-secondary)' }}>
                      ...
                      <div className="mt-3 h-2 overflow-hidden rounded-full" style={{ background: 'color-mix(in srgb, var(--revka-bg-panel-strong) 86%, transparent)' }}>
                        <div style={{ width: `${Math.max(share, 2)}%`, background: 'var(--revka-signal-selected)', height: '100%' }} />
                      </div>
                    </button>
                  );
                })
          ...
          <div className="flex flex-col gap-4 lg:overflow-y-auto lg:min-h-0">
            <Panel className="p-4" variant="utility">
              <div className="revka-kicker">{t('cost.selected_model')}</div>
              {!selectedModel ? (
                <div className="mt-4">
                ...
                <div className="mt-3 space-y-3">
                  <div className="text-sm font-semibold" style={{ color: 'var(--revka-text-primary)' }}>{selectedModel.model}</div>
            ...
            <Panel className="p-4" variant="secondary">
              <div className="revka-kicker">{t('cost.interpretation')}</div>
