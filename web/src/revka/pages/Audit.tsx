...
function eventTypeColor(eventType: string): string {
  switch (eventType) {
    ...
    case 'policy_violation':
      return 'var(--revka-status-danger)';
    case 'security_event':
      return 'var(--revka-status-warning)';
    case 'auth_success':
      return 'var(--revka-status-success)';
    default:
  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] flex-col gap-3 lg:h-[calc(100vh-6rem)]">
      <PageHeader
        ...
        actions={(
          <>
            <button className="revka-button" onClick={loadEvents}>
              <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />
              {t('audit.refresh')}
            </button>
            <button className="revka-button" onClick={handleVerify} disabled={verifying}>
      ...
      <div className="grid gap-4 grid-cols-1 lg:min-h-0 lg:flex-1 lg:grid-cols-[18rem_minmax(0,1fr)]">
        <Panel className="p-4" variant="secondary">
          <div className="revka-kicker">{t('audit.filters')}</div>
          <div className="mt-3 space-y-3">
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="revka-input">
            ...
            {chainStatus ? (
              <div className="rounded-[12px] border p-3 text-sm" style={{ borderColor: chainStatus.verified ? 'color-mix(in srgb, var(--revka-status-success) 24%, transparent)' : 'color-mix(in srgb, var(--revka-status-danger) 24%, transparent)' }}>
                <div className="flex items-start gap-2">
                  {chainStatus.verified ? (
                    <CheckCircle className="mt-0.5 h-4 w-4" style={{ color: 'var(--revka-status-success)' }} />
                  ) : (
                    <XCircle className="mt-0.5 h-4 w-4" style={{ color: 'var(--revka-status-danger)' }} />
                  )}
                  <div>
                    <div style={{ color: chainStatus.verified ? 'var(--revka-status-success)' : 'var(--revka-status-danger)' }}>
        ...
        <Panel className="p-5 lg:overflow-y-auto">
          <div className="revka-kicker">{t('audit.stream_title')}</div>
          ...
          <div className="mt-5 space-y-3">
            {loading ? (
              ...
              events.map((event) => (
                <div key={event.event_id} className="rounded-[14px] border p-4" style={{ borderColor: 'var(--revka-border-soft)' }}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      {event.actor ? (
                        <span className="text-xs" style={{ color: 'var(--revka-text-secondary)' }}>
                          {event.actor.channel}{event.actor.username ? ` · ${event.actor.username}` : ''}
                        </span>
                      ) : null}
                    </div>
                    <span className="text-xs" style={{ color: 'var(--revka-text-faint)' }}>{formatRelative(event.timestamp)}</span>
                  </div>
                  <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4 text-xs">
                    <div>
                      <div className="revka-kicker">{t('audit.col.sequence')}</div>
                      <div className="mt-1" style={{ color: 'var(--revka-text-primary)' }}>{event.sequence}</div>
                    </div>
                    <div>
                      <div className="revka-kicker">{t('audit.col.action')}</div>
                      <div className="mt-1" style={{ color: 'var(--revka-text-primary)' }}>{event.action?.command || '—'}</div>
                    </div>
                    <div>
                      <div className="revka-kicker">{t('audit.col.result')}</div>
                      <div className="mt-1" style={{ color: event.result?.success ? 'var(--revka-status-success)' : 'var(--revka-text-secondary)' }}>
                        {event.result?.success ? t('audit.result_success') : event.result?.error || t('audit.result_recorded')}
                      </div>
                    </div>
                    <div>
                      <div className="revka-kicker">{t('audit.col.hash')}</div>
                      <div className="mt-1 font-mono" style={{ color: 'var(--revka-text-faint)' }}>{event.entry_hash.slice(0, 16)}</div>
                    </div>
                  </div>
                  {event.security.policy_violation ? (
                    <div className="mt-3 inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ color: 'var(--revka-status-danger)', background: 'color-mix(in srgb, var(--revka-status-danger) 10%, transparent)' }}>
