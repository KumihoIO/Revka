/**
 * WorkflowEditor — P0 redesign on Revka dashboard tokens.
 *
 * Replaces the legacy `web/src/components/workflows/WorkflowEditor.tsx`.
 * Reuses the revka workflow data layer (yamlSync) so the YAML schema and
...
import { resolveCssVar } from '@/revka/lib/orchestration';
import {
  ADD_STEP_EVENT,
...
import '@/revka/styles/editor-chrome.css';
...
function RunLogDialog({
  ...
  return (
    ...
    >
      ...
      >
        <Crosshair size={14} style={{ color: 'var(--revka-signal-network)' }} />
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--revka-text-primary)' }}>
            Run to here
          </div>
          <div
            style={{
      ...
      <div style={{ overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        >
          <span style={{ color: 'var(--revka-text-faint)' }}>Run ID</span>
          <span
            style={{
          ...
          <span style={{ color: 'var(--revka-text-faint)' }}>Started</span>
          <span style={{ color: 'var(--revka-text-secondary)' }}>
            {formatRunTimestamp(detail?.started_at || runLog.startedAt)}
          </span>
          <span style={{ color: 'var(--revka-text-faint)' }}>Progress</span>
          <span style={{ color: 'var(--revka-text-secondary)' }}>
        ...
        {targetError ? (
          <div
            style={{
              padding: 10,
              borderRadius: 10,
              border: '1px solid color-mix(in srgb, var(--revka-status-danger) 34%, transparent)',
              background: 'color-mix(in srgb, var(--revka-status-danger) 10%, transparent)',
        ...
        <div>
          <div className="revka-kicker" style={{ marginBottom: 8 }}>Target Output</div>
          {targetOutput ? (
            <pre
              style={{
                margin: 0,
        ...
        <div>
          <div className="revka-kicker" style={{ marginBottom: 8 }}>Steps</div>
          {steps.length > 0 ? (
                ...
                <div
                  ...
                  style={{
                    ...
                    border: step.step_id === runLog.targetStepId
                      ? '1px solid color-mix(in srgb, var(--revka-signal-network) 48%, transparent)'
                      : '1px solid var(--revka-border-soft)',
                    background: step.step_id === runLog.targetStepId
                      ? 'color-mix(in srgb, var(--revka-signal-network) 10%, transparent)'
                      : 'var(--revka-bg-surface)',
                  }}
                >
                  <span
                    style={{
...
const WIRE_STYLE_STORAGE_KEY = 'revka.workflowEditor.wireStyle';
const YAML_PANEL_WIDTH_STORAGE_KEY = 'revka.workflowEditor.yamlPanelWidth';
const DETAILS_PANEL_WIDTH_STORAGE_KEY = 'revka.workflowEditor.detailsPanelWidth';
...
function runStatusColor(status: string | undefined): string {
  ...
  if (['completed', 'success'].includes(normalized)) return 'var(--revka-signal-selected)';
  if (['failed', 'error', 'cancelled', 'canceled'].includes(normalized)) return 'var(--revka-status-danger)';
  if (['running', 'starting', 'queued', 'pending'].includes(normalized)) return 'var(--revka-signal-live)';
  return 'var(--revka-text-faint)';
}
...
function WorkflowEditorInner({
  ...
  // Agent picker state — driven by `revka:open-agent-picker` events from
  ...
  // ── Add a new step (called via revka:add-step event) ────────────────
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button type="button" onClick={onCancel} className="revka-button" style={{ padding: '6px 12px', fontSize: 12 }}>
            Cancel
          </button>
          <button
            type="button"
      ...
      {error && (
        <div
          style={{
            ...
            padding: '8px 12px',
            borderRadius: 10,
            border: '1px solid color-mix(in srgb, var(--revka-status-danger) 32%, transparent)',
            background: 'color-mix(in srgb, var(--revka-status-danger) 10%, transparent)',
            color: 'var(--revka-status-danger)',
            fontSize: 13,
            display: 'flex',
      ...
      {warning && (
        <div
          style={{
            ...
            padding: '8px 12px',
            borderRadius: 10,
            border: '1px solid color-mix(in srgb, var(--revka-status-warning) 32%, transparent)',
      ...
      {pendingRemoteUpdate ? (
        ...
        >
          <Radio size={14} style={{ color: 'var(--revka-signal-network)' }} />
          <span
            style={{
              flex: 1,
              fontSize: 13,
      ...
      >
        ...
        <Panel className="overflow-hidden">
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            >
              <div className="revka-kicker">Graph</div>
              <span style={{ fontSize: 11, color: 'var(--revka-text-faint)', marginLeft: 6 }}>
                {liveNodes.length} step{liveNodes.length === 1 ? '' : 's'} · {edges.length} edge
                {edges.length === 1 ? '' : 's'}
                {cycleDetected ? (
                  <>
                    {' · '}
                    <span style={{ color: 'var(--revka-status-danger)' }}>cycle detected</span>
                  </>
                ) : null}
              </span>
              <div style={{ flex: 1 }} />
            ...
            <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                ...
                <>
                  <div
                    style={{
                      ...
                      flexShrink: 0,
                      borderRight: '1px solid var(--revka-border-soft)',
                      display: 'flex',
                      flexDirection: 'column',
                      minHeight: 0,
                      background: 'var(--revka-bg-surface)',
                  ...
                  >
                    ...
                    >
                      <span className="revka-kicker">YAML</span>
                      <button
                        type="button"
                    ...
                    {yamlDirty ? (
                      <div
                        ...
                        style={{
                          ...
                          color: 'var(--revka-status-warning)',
                          background: 'color-mix(in srgb, var(--revka-status-warning) 8%, transparent)',
                        }}
                      >
                    ...
                    <div
                      style={{
                        borderBottom: '1px solid var(--revka-border-soft)',
                        padding: '8px 12px',
                        background: 'color-mix(in srgb, var(--revka-bg-elevated) 70%, transparent)',
                      }}
                    >
                      <div
                        style={{
              ...
              >
                ...
                >
                  <Background color="var(--revka-grid-line, var(--pc-border))" gap={20} size={1} />
                  <Controls
                  ...
                  {liveNodes.length > 0 && liveNodes.length <= 40 && (
                    <MiniMap
                      ...
                      nodeColor={() => resolveCssVar('var(--revka-signal-selected)')}
                    />
                  )}
                </ReactFlow>
  ...
  return (
    <button
      ...
      style={{
        ...
        border: 0,
        background: 'transparent',
        color: disabled ? 'var(--revka-text-faint)' : 'var(--revka-text-primary)',
        fontSize: 12,
        textAlign: 'left',
...
function ContextMenuSeparator() {
  return <div style={{ height: 1, margin: '4px 6px', background: 'var(--revka-border-soft)' }} />;
}
...
function WorkflowSettingsPanel({
  ...
  return (
    <Panel variant="primary" className="overflow-hidden">
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--revka-border-soft)' }}>
          <div className="revka-kicker">Workflow Settings</div>
        </div>
