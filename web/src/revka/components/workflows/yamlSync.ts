...
export interface StepRunInfo {
  ...
  action?: string;
  duration_s?: number;
  trust_score?: number; // 0.0–1.0 from Revka/AgentTrust
...
export const GATE_EDGE_STYLES = {
  true: { stroke: 'var(--revka-status-success)', strokeWidth: 2 },
  false: { stroke: 'var(--revka-status-danger)', strokeWidth: 2 },
  default: { stroke: 'var(--revka-signal-selected)', strokeWidth: 2 },
} as const;
    ...
    for (let ci = 0; ci < children.length; ci++) {
      ...
      if (ci === 0) {
        ...
        pushEdge({
          ...
          selectable: true,
          interactionWidth: 20,
          style: { stroke: 'var(--revka-signal-live)', strokeWidth: 2 },
          data: { synthetic: true },
        });
      } else {
