...
} from '@/revka/hooks/useAgentChatSession';
import {
  matchCommands,
  parseInput,
  resolveCommand,
  ...
  type SlashThemeName,
} from '@/revka/components/assistant/slashCommands';
import SlashCommandMenu from '@/revka/components/assistant/SlashCommandMenu';
import ActivityCard from '@/revka/components/assistant/ActivityCard';
...
function architectSessionIdFor(workflowKref: string | null): string {
  const key = workflowKref
    ? `revka.architect.session_id:${workflowKref}`
    : 'revka.architect.session_id:new';
  try {
...
function buildContextPreface(workflowName: string): string {
  return [
    'You are the Architect for the Revka workflow editor.',
    '',
    ...
    '- Whenever a step\'s prompt/command/template references another step via placeholders such as `STEP_ID.output_data.artifact_path`, `STEP_ID.output`, or `STEP_ID.status`, also list that step ID in `depends_on`. The runtime infers missing deps automatically, but explicit declarations are clearer.',
    ...
    '  RIGHT:',
    '    prompt: "Combine these two reports.\\n\\nA artifact: ${research_a.output_data.artifact_path}\\n\\nB artifact: ${research_b.output_data.artifact_path}\\n\\nRead both artifacts before synthesizing."',
    '    depends_on: [research_a, research_b]',
    '  The runtime cannot auto-inject prior results into prompts. If an agent step needs information from another step, prefer `${other_step.output_data.artifact_path}` and tell the agent to read that file for full context. Use `${other_step.output}` only for short inline handoffs. The validator rejects agent steps with depends_on entries that are not referenced in the prompt.',
    '',
...
function ArchitectChatSurface({
  ...
  const handleToolResult = useCallback(
    (evt: ToolResultEvent) => {
      ...
      // (e.g. `revka-operator__propose_workflow_yaml`). Match on the
    ...
    <>
        ...
        <div className="h-[2px] overflow-hidden" style={{ background: 'var(--revka-bg-surface)' }}>
          <div
            ...
            style={{
              ...
              width: '40%',
              animation: 'revka-architect-sweep 1.4s ease-in-out infinite alternate',
            }}
          />
        </div>
      )}
      ...
      {error && (
        <div
          ...
          style={{
            borderColor: 'color-mix(in srgb, var(--revka-status-danger) 32%, transparent)',
...
└──────────────────────────────┘`}
          ...
          <div className="space-y-3">
            ...
            {typing && (streamingContent || streamingThinking) && (
              <div className="whitespace-pre-wrap break-words">
                <span
                  style={{ color: 'var(--revka-signal-live)', fontWeight: 600 }}
                >
                  architect {'>'}{' '}
                </span>
                <span style={{ color: 'var(--revka-signal-live)' }}>
                  {streamingContent || '…'}
                </span>
              </div>
            )}
      ...
      >
        ...
        >
          ...
          >
            {'>'}<span className="revka-cursor-blink">_</span>
          </span>
          <textarea
            ref={inputRef}
            rows={1}
            ...
            style={{
              ...
              caretColor: 'var(--revka-signal-network)',
              maxHeight: '6rem',
              fontSize: 16,
            }}
          />
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            borderColor: 'var(--revka-border-soft)',
            color: 'var(--pc-text-faint, var(--revka-text-faint))',
            background: 'var(--revka-bg-surface)',
          }}
        >
          Powered by Operator
      ...
      <style>{`
        @keyframes revka-architect-sweep {
