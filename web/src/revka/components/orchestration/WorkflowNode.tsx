...
function WorkflowNode({
  ...
  return (
    <div
      ref={ref}
      className="rounded-[8px] border px-4 py-3 shadow-sm flex flex-col"
      title={[
      ...
      style={{
        ...
        minWidth: 220,
        maxWidth: 280,
        borderColor: selected ? operationalAccent : 'color-mix(in srgb, var(--revka-border-soft) 75%, transparent)',
        ...
        boxShadow: selected
          ...
          : data.failing
            ? `0 0 18px color-mix(in srgb, var(--revka-status-danger) 22%, transparent)`
            : data.blocked
              ? `0 0 14px color-mix(in srgb, var(--revka-status-warning) 18%, transparent)`
    ...
    >
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            <span
              ...
              style={{ background: 'color-mix(in srgb, var(--revka-bg-elevated) 85%, transparent)', color: operationalAccent }}
            >
              {data.type}
            </span>
            {data.runInfo ? (
            ...
            {data.failing ? (
              <span className="rounded-md px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ background: 'color-mix(in srgb, var(--revka-status-danger) 14%, transparent)', color: 'var(--revka-status-danger)' }}>
                failure path
              </span>
            ) : null}
            {data.blocked ? (
              <span className="rounded-md px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ background: 'color-mix(in srgb, var(--revka-status-warning) 14%, transparent)', color: 'var(--revka-status-warning)' }}>
                blocked
              </span>
            ) : null}
          </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        {data.runInfo?.skills?.length ? (
          <span className="rounded-md px-1.5 py-0.5 text-[10px] font-medium" style={{ background: 'color-mix(in srgb, var(--revka-status-warning) 18%, transparent)', color: 'var(--revka-status-warning)' }}>
      ...
      {branchSummary ? (
        <div className="mt-2 rounded-md px-2 py-1 text-[10px] leading-4" style={{ background: 'color-mix(in srgb, var(--revka-status-success) 12%, transparent)', color: 'var(--revka-text-secondary)' }}>
          {branchSummary}
        </div>
      ) : null}

      {failureReason ? (
        <div className="mt-2 rounded-md px-2 py-1 text-[10px] leading-4" style={{ background: 'color-mix(in srgb, var(--revka-status-danger) 12%, transparent)', color: 'var(--revka-status-danger)' }}>
          {failureReason}
        </div>
      ) : null}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: selected ? `0 0 0 1px ${accent}, 0 0 24px color-mix(in srgb, ${accent} 22%, transparent)` : 'var(--revka-shadow-panel)',
      }}
    >
