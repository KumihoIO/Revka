      ...
      {matches.map((cmd, i) => {
        ...
        return (
          <button
            ...
            style={{
              background: active ? 'color-mix(in srgb, var(--revka-signal-live) 12%, transparent)' : 'transparent',
              color: active ? 'var(--revka-text-primary)' : 'var(--revka-text-secondary)',
            }}
          >
            <span className="flex min-w-0 items-baseline gap-2">
              <span className="font-mono font-semibold" style={{ color: active ? 'var(--revka-signal-live)' : 'var(--revka-text-primary)' }}>
                /{cmd.name}
              </span>
              {cmd.args ? (
                <span className="font-mono text-[10px]" style={{ color: 'var(--revka-text-faint)' }}>
                  {cmd.args}
                </span>
              ) : null}
            ...
            <span className="truncate text-[11px]" style={{ color: 'var(--revka-text-muted)' }}>
              {cmd.description}
            </span>
          </button>
        );
