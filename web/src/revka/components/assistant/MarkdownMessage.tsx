...
function CopyableCodeBlock({ code, language }: { code: string; language?: string }) {
  ...
  return (
    <div className="group/code relative my-2 overflow-hidden rounded-md border" style={{ borderColor: 'var(--revka-border-soft)', background: 'var(--revka-bg-base)' }}>
      <div className="flex items-center justify-between border-b px-2 py-1" style={{ borderColor: 'var(--revka-border-soft)', color: 'var(--revka-text-faint)' }}>
        ...
        <button
          ...
          style={{ color: copied ? 'var(--revka-status-success)' : 'var(--revka-text-muted)' }}
        >
          {copied ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
          {copied ? t('agent.copied') : t('agent.copy')}
        </button>
      </div>
      <pre className="m-0 max-h-[28rem] overflow-auto p-3 font-mono text-[0.86em] leading-6" style={{ color: 'var(--revka-text-secondary)', textShadow: 'none' }}>
        <code>{code}</code>
      </pre>
    </div>
  );
          ...
          code({ children, className, node: _node, ...props }) {
            ...
            return (
              <code
                ...
                className={className}
                style={{
                  background: 'color-mix(in srgb, var(--revka-bg-surface) 75%, transparent)',
