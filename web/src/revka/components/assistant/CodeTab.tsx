...
const TOOL_OPTIONS: ToolOption[] = [
  ...
  { key: 'operator', label: 'Revka Operator' },
];

const RECENT_REPOS_KEY = 'revka-code-recent-repos';
...
function mcpOverrideBaseUrl(): string | null {
  const env = (import.meta.env.VITE_REVKA_MCP_URL as string | undefined)?.trim();
...
function buildXtermTheme(colors: SchemeColors): Record<string, string> {
  return {
    ...
    cursorAccent: '#0b0d10',
    selectionBackground: 'rgba(111,131,214,0.18)',
...
function McpStatusStrip({
  ...
  const stripStyle: React.CSSProperties = {
    ...
    background: 'color-mix(in srgb, var(--revka-bg-surface) 92%, transparent)',
    minHeight: 64,
    maxHeight: 96,
  };
  ...
  if (status === 'offline') {
    return (
      <div
        ...
        style={stripStyle}
      >
        <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: 'var(--revka-text-faint)' }} />
        <span style={{ color: 'var(--revka-text-faint)' }}>
    ...
    return (
      ...
      >
        <span
          ...
          style={{ background: status === 'open' ? colors.primary : 'var(--revka-text-faint)' }}
        />
        <span style={{ color: 'var(--revka-text-muted)' }}>
          waiting for Revka tool activity
        </span>
      </div>
    );
  }
        ...
        return (
          <div
            ...
            style={{
              minWidth: 150,
              maxWidth: 240,
              borderColor: faded ? 'var(--revka-border-soft)' : 'color-mix(in srgb, ' + colors.primary + ' 40%, var(--revka-border-soft))',
          ...
          >
            <div
              ...
              style={{
                color: faded ? 'var(--revka-text-faint)' : colors.primary,
            ...
            <div
              ...
              style={{ color: faded ? 'var(--revka-text-faint)' : 'var(--revka-text-primary)' }}
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    background: faded ? 'var(--revka-text-faint)' : colors.primary,
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col" style={{ display: visible ? 'flex' : 'none' }}>
      ...
      >
        ...
        <span
          className="truncate"
          style={{ color: 'var(--revka-text-muted)', maxWidth: '40ch' }}
          title={session.cwd}
        >
          {session.cwd}
        </span>
        ...
        <button
          ...
          style={{ color: 'var(--revka-text-secondary)', border: '1px solid var(--revka-border-soft)' }}
          title="End session"
        >
...
export default function CodeTab({
  ...
  const startSession = useCallback(async () => {
    ...
    if (tool === 'operator') {
      // Revka Operator → delegate to a Chat tab, do not spawn a PTY.
  ...
  const statusText = useMemo(() => {
    if (daemonStatus === 'checking') return 'Revka MCP: checking…';
    if (daemonStatus === 'ready') return 'Revka MCP: ready';
    const suffix = daemonReason ? ` (${daemonReason})` : '';
    return `Revka MCP: not running — start revka-mcp first${suffix}`;
  }, [daemonStatus, daemonReason]);

  const statusColor = useMemo(() => {
    if (daemonStatus === 'ready') return 'var(--revka-status-success)';
    if (daemonStatus === 'missing') return 'var(--revka-status-danger)';
    ...
    >
      <div className="mx-auto w-full max-w-md space-y-4">
        <div>
          ...
          <p className="mt-1 text-[11px]" style={{ color: 'var(--revka-text-muted)' }}>
            Start a CLI coding agent in a terminal, wired to the shared Revka MCP daemon.
          </p>
        </div>
        ...
        <div
          ...
          style={{
            borderColor: 'var(--revka-border-soft)',
            background: 'color-mix(in srgb, var(--revka-bg-surface) 95%, transparent)',
          }}
        >
          <span
          ...
          />
          <span style={{ color: 'var(--revka-text-secondary)' }}>{statusText}</span>
        </div>
        ...
        <div>
          <label
            ...
            style={{ color: 'var(--revka-text-faint)', fontSize: '10px' }}
          >
            Tool
          </label>
          <select
          ...
          >
            {TOOL_OPTIONS.map((opt) => (
              <option key={opt.key} value={opt.key} style={{ background: 'var(--revka-bg-panel-strong)' }}>
                {opt.label}
              </option>
            ))}
          </select>
          ...
          <div>
            ...
            {recentRepos.length > 0 && (
              <div className="mt-2">
                ...
                >
                  <option value="" style={{ background: 'var(--revka-bg-panel-strong)' }}>
                    Select recent…
                  </option>
                  {recentRepos.map((p) => (
                    <option key={p} value={p} style={{ background: 'var(--revka-bg-panel-strong)' }}>
                      {p}
                    </option>
                  ))}
                </select>
        ...
        {tool === 'operator' && (
          <p className="text-[11px]" style={{ color: 'var(--revka-text-muted)' }}>
            Revka Operator runs as a chat conversation, not a PTY. Starting it will open a Chat
