...
export const STORAGE_KEY = 'revka-theme';
export const DEFAULT_SKIN_ID = 'revka';
...
export function loadStored(): StoredTheme {
  try {
    ...
    if (raw) {
      ...
      const hasStoredSkin = Object.prototype.hasOwnProperty.call(parsed, 'activeSkinId');
      const activeSkinId = typeof parsed.activeSkinId === 'string' && parsed.activeSkinId.trim()
        ? parsed.activeSkinId
        : hasStoredSkin ? null : DEFAULTS.activeSkinId;
