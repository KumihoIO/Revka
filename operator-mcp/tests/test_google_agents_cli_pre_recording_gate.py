...
def _complete_track3_manifest(service_url: str) -> dict:
    return {
        "schema_version": 1,
        "scenario": {
...
def _write_complete_track3_evidence(evidence_dir: Path) -> None:
    ...
    files = {
        ...
        "a2a/message-send-response.json": json.dumps(
            {
                "result": {
                            ...
                            "parts": [
                                {
                                    "type": "text",
                                    "text": (
                                        "Incident plan: business impact is high. Coordinate "
                                        "specialized agents through A2A, inspect Google Cloud "
                                        "evidence and Cloud Logging, enforce approval boundary, "
                                        "rollback to the last known good revision, and deliver "
                                        "the final operator recommendation."
                                    ),
                                }
                            ]
                        }
        ...
        "runtime/healthz.json": json.dumps(
            {
                "ok": True,
    ...
    for rel, text in files.items():
        _write(evidence_dir / rel, text)
    (evidence_dir / "manifest.json").write_text(
        json.dumps(_complete_track3_manifest(service_url)),
        encoding="utf-8",
    )
...
def test_pre_recording_gate_track3_reports_strict_final_ready_without_agents_cli_auth(tmp_path):
    evidence_dir = tmp_path / "track3-evidence"
    output = tmp_path / "report.json"
    output_dir = tmp_path / "artifacts"
    _write_complete_track3_evidence(evidence_dir)
    ...
    assert statuses["track3_demo_probe"] == "pass"
    assert statuses["track3_evidence_gate"] == "pass"
...
def test_pre_recording_gate_track3_fails_when_evidence_is_missing(tmp_path):
    output = tmp_path / "report.json"
    ...
    result = subprocess.run(
        [
            ...
            "track3",
            "--evidence-dir",
            str(tmp_path / "missing-track3-evidence"),
            "--output",
            str(output),
        ],
        check=False,
    ...
    report = json.loads(output.read_text(encoding="utf-8"))
    track3 = next(item for item in report["checks"] if item["name"] == "track3_evidence_gate")
    assert track3["status"] == "fail"
    assert any("Track 3 evidence gate exited 1" in item for item in track3["failures"])
    assert any("missing manifest" in item for item in track3["global_failures"])
    assert any("track3_evidence_gate global failures:" in item for item in report["strict_final_blockers"])


def test_pre_recording_gate_track3_final_mode_fails_when_evidence_is_skipped(tmp_path):
    output = tmp_path / "report.json"
    ...
    assert "Track 3 evidence validation was skipped" in report["strict_final_blockers"]
    statuses = {item["name"]: item["status"] for item in report["checks"]}
    assert statuses["track3_demo_probe"] == "pass"
    assert statuses["track3_evidence_gate"] == "skip"
