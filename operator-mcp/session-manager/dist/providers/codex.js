...
export function createCodexSession(config, onEvent) {
    ...
    const runPrompt = (prompt) => {
        ...
        if (config.agentType === "codex") {
            binary = "codex";
            args.push("--ask-for-approval", "never", "--sandbox", "danger-full-access", "exec", "--json", "--skip-git-repo-check");
            if (config.model) {
        ...
        const isPlainTextAgent = config.agentType === "agy" || config.agentType === "cursor";
        // Keep the prompt (always the final arg when passed on argv) out of the log.
        const logArgs = args.length > 0 && args[args.length - 1] === effectivePrompt
            ? args.slice(0, -1)
            : args;
