...
fn model_supports_temperature(model: &str) -> bool {
    ...
    // Anthropic removed `temperature` from adaptive-thinking models. Keep the
    // 4.x prefixes specific so Opus 4.5/4.6, Sonnet 4.6, and Haiku 4.5 retain
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = AnthropicProvider::new(None);
        let result = p
            .chat_with_system(Some("You are Revka"), "hello", "claude-3-opus", 0.7)
            .await;
        assert!(result.is_err());
    }
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are Revka".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are Revka\""));
    }
    ...
    fn model_supports_temperature_helper() {
        ...
        assert!(!model_supports_temperature("claude-sonnet-5"));
        assert!(!model_supports_temperature("anthropic/claude-sonnet-5"));
        assert!(!model_supports_temperature("claude-opus-5"));
