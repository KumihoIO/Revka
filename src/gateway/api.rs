...
/// Returns true when `X-Construct-Service-Token` matches `state.service_token`.
///
/// The service token is generated by the gateway at startup and written to
/// `~/.construct/service-token`; it is the credential trusted local sidecars
...
///   - `X-Construct-Service-Token: <token>` — gateway's own credential for trusted
...
pub(super) fn require_auth(
    ...
    Err((
        ...
        Json(serde_json::json!({
            "error": "Unauthorized — pair first via POST /pair (Authorization: Bearer <token>) or present X-Construct-Service-Token"
        })),
    ))
}
...
mod tests {
    ...
    fn require_auth_does_not_accept_empty_service_token_when_unconfigured() {
        ...
        // `X-Construct-Service-Token: ` header must not authenticate.
        let cfg = crate::config::Config::default();
        let mut state = test_state(cfg);
        state.pairing = Arc::new(PairingGuard::new(true, &["PAIR-TOKEN".to_string()]));
