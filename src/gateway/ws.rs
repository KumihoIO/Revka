...
fn page_context_hint(page: &str) -> Option<&'static str> {
    match page {
        "agent_pool" => Some(concat!(
            ...
            "expertise areas, preferred agent type/model (claude, codex, or google_agents; ADK projects usually own their model config), ",
        ...
        "workflows" => Some(concat!(
            ...
            "      agent_hints: [researcher]  # hints for operator: coder | researcher | reviewer | google_agents | adk\n",
            "      depends_on: []\n",
            "    - id: code_step\n",
            "      name: Implementation\n",
            "      action: code\n",
            ...
            "Use explicit `type: agent` + `agent_type: google_agents` for Google Agents CLI / ADK steps.\n",
