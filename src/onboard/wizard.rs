...
fn prompt_existing_kumiho_token() -> Result<Option<String>> {
    loop {
        print_bullet(&t!("memory-kumiho-token-hidden-note"));
        let raw_token: String = dialoguer::Password::new()
            .with_prompt(format!("  {}", t!("memory-kumiho-token")))
            .allow_empty_password(true)
            .interact()?;
        let Some(token) = normalize_pasted_kumiho_service_token(&raw_token) else {
            return Ok(None);
        };
